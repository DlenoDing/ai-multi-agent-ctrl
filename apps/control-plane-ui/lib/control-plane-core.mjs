...
const DEFAULT_SYSTEM_RULES = [
  ...
  {ruleId: "sys.review-dual-track", title: "互审双轨：既审当前方案也另寻更优", content: "任何互审/评审都必须同时走两条轨道，缺一不可，只做其中一条即为不合格评审。轨道一（方案内审查）：按当前方案的目标、边界、验收标准审查其执行是否正确、完整、有证据。轨道二（方案外求优）：跳出当前方案本身，重新回到【原始问题与目标】，判断这个方案本身是不是解决该问题的正确/最优路径，并至少给出一个可比较的替代方案，或明确说明已考察过哪些替代路径、为何当前方案更优。设立轨道二的原因：只沿既定方案往下审，会把一个本来就错的方向越做越精细——评审越勤，偏差越大；必须周期性回到问题本身，才可能发现方向错了，而不只是执行得不够好。触发即强制：方案/拓扑选择、任务拆分、验收、规则变更等核心决策的互审，必须在结论里显式记录 alternativesConsidered（考察过的替代路径及取舍理由）；为空视为评审未完成，不得据此推进。与既有规则的关系：轨道二只产出【提案与意见】，不构成擅自改动。发现更优方案时，一律通过人工确认通道提出（confirmation_analyze 的 better_alternative / incorrect），由人决定是否改变方案；AI 不得据自己的更优判断绕过已定稿方案或自行扩大改动范围——那属于 sys.scope-convergence 与角色漂移守卫的管辖，仍然禁止。即：想得更宽是义务，做得更宽要授权。最终方案的判准（三者都要，缺一即为不合格评审）：简单、高性能、稳定。轨道二比较替代路径时必须逐条给出这三项的判断，不得只谈其中一项就下结论；alternativesConsidered 的每一条都要写明它在这三项上相对当前方案的取舍。若最终方案在某一项上明显劣于某个替代路径，必须显式说明为什么这个代价可接受——不说明即视为评审未完成。三者冲突时的排序：先保证稳定（正确性与可恢复性），再保证简单，最后才是性能；但不得用「以后再优化」把性能问题记成技术债而不给出量级判断——说不清量级就是没做轨道二。最常见的失效是拿【简单】去换尚未发生的通用性/扩展性：除非有已确认的需求，不得为假想场景增加机制、抽象层、配置项或状态；每多一个可配置项、每多一条分支，都要在结论里回答「谁会用它、不加会怎样」（与 sys.scope-convergence 同向）。性能与稳定性的判断必须落在真实运行面上（见 sys.owner-path-verification）：「理论上更快/更稳」不构成证据，须给出量级与它来自哪条真实路径的观测。简单性的判准不是代码行数，而是【要理解它需要同时在脑子里放几件事】、【出错时能不能一眼看出是哪一步】、【删掉它需要改几处】。"},
