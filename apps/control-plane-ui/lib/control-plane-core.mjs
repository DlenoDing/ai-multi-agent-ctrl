...
import { createCommandBus as createCommandBusModule } from "./command-bus.mjs";
import { createRuntimeIssueTracker } from "./runtime-issue-tracker.mjs";
import {
...
export function dispatchContractSummary(state, dispatch) {
  ...
  return {
    ...
    roleSkill: skill ? {roleSkillId: skill.roleSkillId || skill.roleSkillRef || null, title: skill.title || skill.name || null,
      contentDigest: skill.contentDigest || skill.roleSkillDigest || null} : null,
...
const runtimeIssueTracker = createRuntimeIssueTracker({
  appendEvent, capCentralCollection, capRetainingPredicate, createId, digestOf, ensureRuntimeCollections, unique
});

export const {collectRuntimeIssue, settleRuntimeIssuePatternForCandidate} = runtimeIssueTracker;
...
const commandBus = createCommandBusModule({
