...
const DEFAULT_SYSTEM_RULES = [
  ...
  {ruleId: "sys.environment-by-config", title: "环境由配置表达", content: "环境统一枚举（项目定义，如 local/dev/test/pre/prod），差异只由配置表达，不用 hostname/IP/容器名/路径/git 分支/机器职责推断业务环境；不擅自清理已有凭据。"},
  {ruleId: "sys.side-effect-authorization", title: "副作用授权与 fail-closed", content: "对会造成不可逆、外部可见或跨环境副作用的动作（正式/生产写入、真实 Provider/支付/KYC/下单等外部调用、删除或清理已有数据/凭据/资源、对外通知）默认禁止；仅在用户对 exact environment/identity/scope/action 明确授权后执行。确定 adapter/契约所必需时只做最小、受控、幂等或可回滚的探针，并预先声明副作用边界与回滚方式。无法正向确证正确性、授权或数据完整性时，对资金/安全/权限/数据破坏类动作一律 fail-closed（拒绝或阻断，而非放行、默认值或吞异常）；命中 P0/安全/资金/数据破坏/证据污染先最小留证 + 止血 + 隔离 + 升级，再按根因修复。"},
  ...
  {ruleId: "sys.independent-review-depth", title: "独立评审语义深度", content: "独立评审不得只看 diff/测试通过：必须判断修复是否落在正确 canonical owner、是否覆盖对应功能族/正常/旧/异常/跨端跨服务消费者/必要负向场景、是否引入上下游或双端不一致、复验范围是否由影响图（upstream/peer/downstream）决定而非改了几个文件。评审会话不得复用被审实现的推导上下文、不得继承其争议结论或 self-pass。命中 symptom-only/sample-only/局部 pass/只列证据不做语义判断/未说明同级面或上下游影响时一律退回补充而非放行。"}
