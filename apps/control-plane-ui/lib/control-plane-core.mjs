...
const DEFAULT_SYSTEM_RULES = [
  ...
  {ruleId: "sys.layered-admission", title: "分层准入与最小复验", content: "cell 因数据量/外部条件窗口/资源额度/设备/完整生产覆盖/多实例 formal gate 暂不能声明正式 pass，不等于不能执行当前条件下的真实验证：必须先产出受限当前条件的真实证据，未满足项精确登记为对应 pending_window/pending_data_volume/resource_queued/verification_incomplete/no_pass_preflight cell，绝不写成父级整体 blocked；条件满足后只复验受影响 cell，不因单个 reverify 触发机械重跑整个大项。准入不得混用 gating 状态字段——条件/资源/外部能力/证据资格是各自正交维度分别记录。FORMAL-PASS-GATE 与 CURRENT-CONDITION-EVIDENCE 必须严格分离：不能声明正式 pass 不得抑制对可跑部分产出当前条件 scoped 证据；任何 defer_to_e2e/延期结论一旦被用作解锁下游 runtime/数据门/资金风险 readiness/正式 pass 的依据，立即升格为 must_reverify_now。"},
  {ruleId: "sys.problem-family-bundle", title: "问题族捆绑修复", content: "非简单问题默认先判定其 problemFamily，不得以「当前报错点→小补丁→独立评审→runtime/E2E 重跑→再发现相邻分支」的流水线推进。触发条件（任一）：同一 owner/组件/能力连续≥2 次 no-pass；问题涉及状态机/拓扑/owner-rebalance/持久状态；评审发现是相邻分支遗漏而非孤立 typo；runtime no-pass 说明 source 测试未覆盖完整运行不变量；用户指出同类反复。触发后暂停小补丁循环，先补完整问题族图→影响面扩展→一次性 bundled 修复（source/config/data/client）→一次族级评审→一次 scoped 复验；评审后仍发现同族相邻遗漏须回同一 bundle 补全再一次性复审，不得另开窄 follow-up。"},
