...
const requestFailureGuidance = window.AIMAC_REQUEST_FAILURE_GUIDANCE;
...
function requestFailureHint(payload) {
  return requestFailureGuidance.hint(payload, {accountName, fmtTime, t, explainCoded, grantRoleLabel});
}
...
function renderLogin() {
  ...
  app.innerHTML = `
    ...
    ${modalHtml}
  `;
  // 登录页也要把弹窗带上：改密成功后会话被清、回到登录页，那句「密码已更新，所有会话已失效，请用新密码重新登录」
  // 就是开在这一屏上的 —— 原先登录页模板不含弹窗，人只看到一个突然出现的登录框，以为出了故障（沙箱实测）。
...
function renderContent() {
  ...
  const pageTabs = !helpActive && !executionObjectOpen && pageWorkspaceTabCount(page) > 1
...
function renderTaskGroupMonitorMatrix(groups, data = {}) {
  return window.AIMAC_TASK_GROUP_INSIGHTS.monitorMatrix(groups, data, {
    state, terminalDispatchStatuses, sessionSettledStatuses: SESSION_SETTLED_STATUSES,
    sinceText, esc, panel
  });
}
/* ---------------- 成员：人工审核 ---------------- */
...
function renderProjectSettings() {
  ...
  const settingsView = {
    repos, baselineData, defaultRoles, resolved, rulesLoaded,
    liveTokens: liveJoinTokenCount(project.id),
    agentStats,
    roleOverlayCount: projectRoleSkillOverlays(project.id).length
  };
  const settingsHelpers = {panel, summaryMetric, jumpModuleCard, projectModuleCard, esc, ruleOwnedAtLayer};
  const settingsWorkspace = window.AIMAC_PROJECT_SETTINGS_WORKSPACE;

  return [
    settingsWorkspace.summary(project, settingsView, settingsHelpers),
    settingsWorkspace.actionBoard(project, settingsView, settingsHelpers),
    settingsWorkspace.boundaryGuide(project, settingsView, settingsHelpers),
    settingsWorkspace.lifecycleGuide(project, settingsView, settingsHelpers),
    panel("项目基础配置", `
    ...
    `, {wide: true}),
    rulesLoaded ? settingsWorkspace.ruleGovernanceOverview(resolved, settingsHelpers) : "",
    rulesLoaded
