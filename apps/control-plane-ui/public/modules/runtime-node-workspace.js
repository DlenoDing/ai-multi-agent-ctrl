(function initRuntimeNodeWorkspace(global) {
  ...
  function events(items, h) {
    ...
    return `<ol class="runtime-node-events">${items.slice(0, 30).map((event) => `<li><span></span><div><div>${h.badge(event.eventType, "blue")} ${h.badge(event.status)}${Number.isFinite(Number(event.progressPercent)) ? ` <strong>${esc(event.progressPercent)}%</strong>` : ""}</div>
      <p>${esc(h.agentEventSummaryZh(event.summary) || "未提供摘要")}</p>${h.evidenceRefsHint(event)}<div class="small muted">${h.fmtTime(event.createdAt)} · ${esc((h.dispatchTaskLabel && h.dispatchTaskLabel(event.dispatchId)) || event.sessionId || "节点事件")}</div></div></li>`).join("")}</ol>`;
  }
  ...
  function render({detail, controls = "", helpers: h} = {}) {
    ...
    // 范围与可见项目给人看名字：原先印的是 org_… / prj_… 这种 id。
    ...
    return `<section class="runtime-node-workspace wide" aria-label="运行节点详情">
      ...
      <div class="runtime-node-columns">
        ...
        <section><span class="governance-eyebrow">调配边界</span><h3>项目与角色</h3><dl class="execution-kv"><dt>注册范围</dt><dd>${esc(scopeText)}</dd><dt>可见项目</dt><dd>${chips((node.effectiveProjectIds || node.projectIds || []).map(projectName), "暂无")}</dd><dt>可承担角色</dt><dd>${chips((node.allowedRoles || []).map((roleId) => (roleId === "*" ? "不限" : h.t(roleId))), "未声明")}</dd><dt>远程 MCP 工具</dt><dd>${collapsibleChips(node.allowedMcpTools || [], "工具", "按任务授权")}</dd></dl></section>
      </div>
