(function () {
  ...
  // 已关闭的组，goalExecutionStatus 与 status 都是 closed：两个徽标并排写着「已关闭 已关闭」。
  // 执行状态与生命周期状态重名时只留生命周期那一个。
  function execBadge(group, h) {
    const exec = group.goalExecutionStatus || "active";
    return exec === group.status ? "" : `${h.badge(exec)} `;
  }
  ...
  function list(groups, h) {
    const rows = groups.map((group) => h.row([
      ...
      `${execBadge(group, h)}${h.badge(group.status)}${group.reviewState === "review_requested" ? ` ${h.badge("review_requested")}` : ""}${group.pauseReason ? `<div class="small muted">${esc(h.t(group.pauseReason))}</div>` : ""}`,
  ...
  function detail(group, body, h) {
    ...
    return `<section class="task-group-object wide" aria-label="任务组工作区">
      <header class="task-group-object-header"><div><button class="secondary-button" data-action="tg-list">返回任务组列表</button>
        <h2>${esc(group.name || group.id)}</h2><div class="record-meta">${h.badge(group.status)} ${h.badge(group.phase)} ${h.badge(group.health)} ${execBadge(group, h)}${group.reviewState === "review_requested" ? ` ${h.badge("review_requested")}` : ""}</div>
