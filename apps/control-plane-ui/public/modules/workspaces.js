(function () {
  ...
  const catalog = {
    ...
    "sys-settings": [pane("runtime", "运行参数", ["运行参数（只读）", "系统设置总览"]), pane("models", "模型能力", ["模型能力注册（只读）"]), pane("skills", "技能源", ["技能源", "角色技能叠加（改动 agent 能力，只读）"]), pane("instruction-efficiency", "指令效率", ["指令压缩指标", "指令信封"]), pane("definitions", "共享定义", ["共享定义归属"]), pane("help", "能力说明")],
  ...
  const legacyPaneAliases = {"monitor:runs": "sessions", "monitor:nodes": "node-control", "monitor:evidence": "checkpoints", "review:decisions": "permissions", "proj-settings:roles": "default-roles", "sys-settings:protocol": "instruction-efficiency"};
