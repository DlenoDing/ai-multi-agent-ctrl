/*
 * 中英映射字典（human-org-console/v1 §6.3）
 * 系统内部标识保持英文，人机界面全部通过 t() 渲染中文。
 * 未映射值兜底显示原文，并 console.warn 一次（开发期发现遗漏）。
 */
(function () {
  "use strict";

  const dict = {
    session: "工作会话",
    dispatch: "Agent 派发",
    /* ---------- 任务契约里的枚举（禁止动作 / 验收要求 / 自检探针 / 执行器类型 / 运行档位）：界面上不出现内部 key ---------- */
    "model-registry": "模型注册",
    checkpoint: "检查点",
    mutate_active_ruleset: "修改生效中的规则集",
    self_patch_control_plane: "自行修改控制面",
    auto_expand_mcp_grant: "自行扩大 MCP 授权",
    external_capability_bypass: "绕过外部能力审批",
    runtime_self_upgrade: "运行时自我升级",
    schema_valid: "结构校验通过",
    checkpoint_registered: "检查点已登记",
    repository_output_target_selected: "已选定仓库产出目标",
    language_policy_satisfied: "满足语言策略",
    model_agent_executor: "模型执行器",
    skill_workset_cache: "技能工作集缓存",
    runtime_local: "本地运行",
    "codex-cli": "Codex 命令行",
    "claude-cli": "Claude 命令行",
    "cursor-cli": "Cursor 命令行",
    /* ---------- 错误码：直接抛给人看的那些 ---------- */
    // 这些码会经 api() 的 t(detail) 渲染成红色提示。未翻译时人看到的是一串英文枚举，
    // 而它们出现的时机恰恰是人最需要看懂的时候（节点没接上、成员停不掉、写请求被重放拦下）。
    // 「已被处置过」这一族：两个人同时在处置同一项时必然撞到。人此刻需要知道的是"不是你操作错了，
    // 是别人先处置了"，而不是一串英文。
    rule_fragments_must_be_an_array: "规则必须以列表提交：传成其它形式会把该层规则整层清空，已拒绝",
    project_has_open_task_groups: "该项目还有未终结的任务组：请先逐个关闭或中止，归档不会替你处置它们",
    config_version_required: "保存整份规则/配置必须带上你读到的那一版：否则会静默覆盖别人在此期间的改动",
    config_version_stale: "这一层配置在你打开之后被别人改过了：直接保存会删掉对方的改动。你刚填的内容还留在表单里，先复制出来，再刷新看对方改了什么，然后重新提交（不刷新的话再点保存还是同一个错——版本号还是旧的）",
    task_group_already_closed: "该任务组已经被关闭过（可能是另一个人刚关的）：定稿归属仍属于最先关闭的那个人，刷新后查看",
    finding_already_resolved: "该发现项已被处置过（可能是另一个人刚处理完）：刷新后查看当前处置结果",
    approval_already_resolved: "该审批请求已被处置过（可能是另一个人刚处理完）：刷新后查看当前结论",
    permission_request_already_resolved: "该授权请求已被处置过（可能是另一个人刚处理完）：你的这次处置未生效，刷新后查看实际结果",
    rule_source_already_settled: "该规则来源已被判定过（可能是另一个人刚处理完）：刷新后查看当前判定",
    execution_topology_already_terminal: "该执行拓扑已到终态，无法再推进：刷新后查看当前状态",
    quality_gate_already_settled: "该质量门已被处置过（可能是另一个人刚处理完）：刷新后查看当前状态",
    review_plan_already_resolved: "该评审计划已被收尾过：刷新后查看当前状态",
    review_bundle_already_resolved: "该评审包已被收尾过：刷新后查看当前状态",
    shared_definition_already_resolved: "该共享定义已被处置过：刷新后查看当前状态",
    system_upgrade_candidate_already_resolved: "该升级候选已被处置过：刷新后查看当前状态",
    shared_definition_resolution_justification_required: "处置共享定义必须写明理由：它会随决定一起留档",
    dispatch_already_completed: "该派发已经结束，无法再对它下达控制指令",
    dispatch_not_resumable: "该派发当前不可恢复：它既不在暂停中，也没有等待恢复的标记",
    execution_topology_not_found: "找不到对应的执行拓扑：它可能已被取代或所属任务组已终结",
    org_last_admin_cannot_be_disabled: "这是本组织最后一个在用的管理员：停用它之后没有人能再管理该组织",
    account_already_retired: "这个账号已经注销过了。注销是终态，没有再来一次这回事，也不能撤销",
    human_stop_requires_human_resume: "这个任务组是人停下来的，只有真人能恢复它 —— 否则「取消归人」这条就等于没有",
    org_last_admin_cannot_be_retired: "这是本组织最后一个在用的管理员：注销它之后没有人能再管理该组织，而注销不可撤销",
    system_last_admin_cannot_be_retired: "这是最后一个在用的系统管理员：注销它之后整个部署永久失去系统层控制权",
    skill_source_sync_failed: "技能源同步失败（仓库取不下来：地址不对 / 需要认证 / 指定的分支不存在 / 网络不通）：该源已标记为 stale，agent 用的还是上一次同步下来的技能",
    heartbeat_timeout: "心跳超时",
    org_status_required: "改组织状态必须明确给出：启用还是停用（不给不会被当作启用）",
    agent_activation_flag_required: "启用还是停用这个智能体，必须明确给出",
    org_quota_update_empty: "没有要改的配额：至少给一项",
    org_member_login_reset_forbidden: "只能重置本组织普通成员的登录：初始管理员由系统管理员重置，已注销的账号不能重置",
    org_initial_admin_reset_forbidden: "只有系统管理员可以重置组织初始管理员登录",
    org_initial_admin_replace: "更换初始组织管理员",
    org_initial_admin_disposition_required: "必须明确处置旧组织管理员",
    org_quota_invalid: "配额必须是 1 到 1000000 的整数（填错的那项没有改）",
    member_status_required: "改成员状态必须明确给出：启用还是停用（不给不会被当作启用）",
    member_permissions_update_empty: "没有要改的内容：权限与默认项目至少给一样",
    account_email_required: "邀请账号必须填邮箱",
    access_grant_subject_and_resource_required: "发授权必须点名给谁（主体）和授权到哪（资源）",
    agent_role_required: "创建智能体必须指定执行角色",
    agent_profile_update_empty: "没有要更新的 Agent 档案字段",
    agent_profile_update: "更新 Agent 档案",
    agent_profile_scope_immutable: "不能通过档案配置改写 Agent 作用域或状态",
    agent_name_required: "Agent 档案名称不能为空",
    agent_model_required: "Agent 模型偏好不能为空",
    approval_request_task_group_required: "提审批必须点名是哪个任务组的事",
    artifact_task_group_required: "登记产出物必须点名是哪个任务组的",
    repository_output_target_task_group_required: "选产出仓库必须点名是哪个任务组的",
    permission_request_task_group_required: "提许可申请必须点名是哪个任务组的",
    execution_topology_task_group_required: "做执行方案必须点名是哪个任务组的",
    derived_task_task_group_required: "派生任务分类必须点名是哪个任务组的",
    review_plan_task_group_required: "建评审计划必须点名是哪个任务组的",
    review_bundle_task_group_required: "交评审包必须点名是哪个任务组的",
    rule_source_task_group_required: "判定规则来源必须点名是哪个任务组的",
    finding_task_group_required: "提发现项必须点名是哪个任务组的",
    role_skill_overlay_task_group_required: "任务组级的角色定制必须点名打在哪个任务组上",
    room_not_specified: "要发到哪个协作室没说清：请给出房间号或任务组",
    task_group_not_found: "找不到这个任务组：系统不会替你挑一个，请确认 taskGroupId",
    work_item_not_found: "这个任务组里找不到该任务：请确认 workItemId",
    agent_control_command_type_required: "给节点下命令必须说清是哪一条命令",
    work_item_title_required: "创建任务必须填标题",
    language_tag_required: "设置统一语言必须选一种语言",
    state_storage_corrupt: "状态文件读不出来（内容已损坏或缺失）：报文里的 file 指出是哪一份，按它恢复或从备份还原；在那之前写操作一律会被拒绝，以免把损坏覆盖成既成事实",
    state_storage_unavailable: "控制面写不进磁盘：请检查运行目录的剩余空间、挂载是否只读、以及进程对它的写权限（读操作不受影响，恢复可写后无需重启）",
    state_view_unknown: "认不出这个视图名：请求里的 view 参数不在支持的取值里（返回体的 supported 列出了全部可选值）",
    mfa_required_but_unavailable: "该账号声明必须二次验证，而本部署尚未实现二次验证：在实现之前不会为它签发会话，请让管理员改回不要求二次验证",
    system_last_admin_cannot_be_disabled: "这是全系统最后一个在用的系统管理员：停用它之后没有人能登录、也没有人能再铸一个，请先邀请另一位系统管理员并让它接受邀请",
    org_member_invite_reissue_not_applicable: "只有尚未接受邀请的成员可以重发邀请",
    bootstrap_init_requires_explicit_confirmation: "重新初始化必须原样输入确认串：这一步会重置控制面数据",
    role_not_allowed_on_node: "该节点的角色范围不含这个派发所需的角色",
    model_not_runnable_on_node: "该节点没有这个派发所需的模型",
    agent_reported_blocked: "执行方报告被阻塞：已交回人工决策，可在「人工指令」里重开或放弃该任务",
    project_repository_not_registered: "本项目尚未登记仓库：到「项目设置」登记之后，任务才有地方写入",
    org_member_invitation_pending: "该成员尚未接受邀请，不能启用；请重新发出邀请或让对方先用一次性令牌登录",
    task_contract_expired: "任务契约已过期：没有节点在有效期内领取这次派发，多半是节点未接入或角色不匹配",
    task_contract_missing: "找不到对应的任务契约：该派发已失去依据，通常发生在任务组配置被改动之后",
    instruction_envelope_task_group_required: "创建指令信封必须点名是哪个任务组的",
    work_item_owner_role_required: "指派必须点名负责角色",
    work_item_owner_role_not_registered: "填写的负责角色未在本任务组登记：先在任务组里加上这个角色，或改选已登记的角色",
    idempotent_result_expired: "这次写入早已完成，但它的结果已过可重放期；请刷新页面查看当前状态，不要重复提交",
    grant_subject_account_not_found: "找不到要授权的账号：请先创建该成员，再授予角色",
    checkpoint_must_use_agent_gateway: "检查点必须由认领该派发的节点经 agent 网关提交",
    grant_subject_account_retired: "这个账号已注销（不可恢复）：给它发授权不会有任何效果",
    // 注销时人没写理由的缺省值 —— 它会直接出现在账号行上，不译就是把码摆给人看。
    human_retire_decision: "人工决定注销（当时没有填写理由）",
    // 已关闭/已中止的任务组不再接受运行控制（暂停/恢复/请求评审/纠偏）。
    task_group_already_terminal: "这个任务组已经结束了（终态），不能再对它做运行控制",
    // 已了结的任务不能再派活（重新派一个已了结的格子不会让它再跑起来）。
    work_item_settled: "这个任务已经了结了（终态），不能再派活",
    // 一台节点可能同时服务多个项目：停它会影响到你没有权限的那几个。
    agent_node_serves_other_projects: "这台节点还在给别的项目干活，而你在那些项目上没有「智能体管理」权限",
    // 共享定义契约要挂在任务组还是项目上，必须说清（不再硬回落到控制面项目）。
    project_required: "要挂在哪个项目下必须说清：填任务组（taskGroupId）或项目（projectId）",
    // 模型选型没选出候选的唯一原因码。它会直接出现在运行时页的选型行上。
    no_candidate_satisfied_hard_constraints: "没有任何候选模型同时满足硬约束（能力档 / 推理档 / 上限）",
    member_default_project_not_found: "默认项目指不到任何项目：请从下拉里选一个，或留空表示不指定",
    member_default_project_outside_organization: "默认项目不属于这个组织：成员进不去，等于没设",
    member_default_project_archived: "这个项目已归档（不能再建新工作）：新成员一进来就无事可做",
    command_timeout_at_invalid: "命令的超时时间不是一个能识别的日期（要 ISO-8601）—— 存进去它就永远不会超时",
    model_capability_observed_at_invalid: "观测时间不是一个能识别的日期（要 ISO-8601，例如 2026-08-23T10:00:00Z）",
    model_capability_provider_class_unknown: "认不出的供应商类别：没有对应的默认模板，注册会缺评分字段而扰乱模型选择。请用受支持的类别（openai / anthropic / … / custom）",
    lease_expired: "租约过期，这份产出目标被顶替",
    skill_source_retired: "技能源已退役",
    agent_control_command_unsupported: "节点认不出这条控制命令（它的运行时版本可能比控制面旧）",
    dispatch_scope_not_active: "这条命令指向的派发已经不是该节点正在跑的那个",
    no_active_dispatch_context: "该节点当时没有在跑任何派发，这条命令没有作用对象",
    grant_resource_type_not_recognized: "认不出这个授权作用域类型：只能是项目、任务组、组织或系统级",
    shared_definition_type_not_recognized: "认不出这个共享定义类型：请从规范登记的类型里选一个（回执的 supported 里列着）",
    shared_definition_conflict_policy_not_recognized: "认不出这个冲突处置策略：请从回执 supported 列出的取值里选",
    room_task_group_settled: "该任务组已终结，它的协作房间不再接收消息",
    project_archived: "该项目已归档（终态，不可撤销），不能再新增任务组、接入智能体或修改配置。要继续这条线，请另建一个项目",
    task_group_settled: "该任务组已终结，不能再往里面新增内容（发现项/许可申请/审批/确认单/执行方案/规则来源）。要继续这条线，请另开一个任务组",
    room_message_payload_too_large: "协作消息过大，已拒绝：请改为提交产出物并在消息里引用它",
    permission_request_status_invalid: "无法识别的处置结果：授权请求只能被批准或拒绝",
    repository_output_target_repository_not_registered_for_project: "该仓库地址不属于本项目已登记的仓库：请先在项目设置里登记它",
    repository_output_target_unsafe_repository_url: "仓库地址不安全，已拒绝：不接受可执行任意命令的传输方式",
    git_verification_transport_policy_missing: "校验用的 git 环境缺少传输白名单，已拒绝校验：这是服务端配置故障，请联系系统管理员",
    /* ---------- 权限码：授权列表与徽标可能直接走 t(code) ---------- */
    "project:view": "查看项目",
    "project:grant": "项目授权管理",
    "project:update": "编辑项目",
    "project:create": "创建项目",
    "project:*": "项目全部权限",
    "member:invite": "邀请成员",
    "agent:activate": "智能体管理",
    "task_group:read": "查看任务组",
    "task_group:control": "任务组执行控制",
    "task_group:review": "任务组人工审核",
    "task_group:monitor": "任务组执行监控",
    "task_group:orchestrate": "任务组编排调度",
    "task_group:checkpoint_submit": "提交检查点",
    "task_group:*": "任务组全部权限",
    "org:member_admin": "组织成员管理",
    "org:project_admin": "组织项目管理",
    "org:*": "组织全部权限",
    "system:account_admin": "系统账号管理",
    "system:bootstrap": "系统初始化",
    "system:model_registry": "模型能力注册",
    "system:skill_sync": "技能源同步",
    "system:*": "系统全部权限",
    /* ---------- 通用执行状态 ---------- */
    queued: "排队中",
    running: "执行中",
    blocked: "受阻",
    completed: "已完成",
    failed: "失败",
    skipped: "已跳过",
    cancelled: "已取消",
    pending: "待确认",
    pending_approval: "待审批",
    // —— 错误码：控制台对任何错误都走 t(payload.error)，缺一条人就看到一句英文。
    // 静态错误码是可枚举的（实测 234 个），因此按全量核对，而不是想到哪条补哪条。
    active_agent_dispatch_required: "需要一个进行中的派发",
    active_session_lease_required: "需要该会话持有生效租约",
    agent_control_command_ack_regression: "控制命令的确认序号回退了（收到比已确认更早的回执）",
    agent_control_command_already_terminal: "该控制命令已处于终态，不能再推进",
    agent_control_command_not_found: "找不到该控制命令",
    agent_control_command_type_invalid: "控制命令类型不合法",
    agent_dispatch_contract_mismatch: "派发与任务契约不匹配",
    agent_node_not_active: "该 agent 节点不处于可用状态",
    agent_runtime_executor_manifest_outside_allowlist: "执行器产出清单落在允许路径之外",
    agent_runtime_executor_missing_artifact_manifest_refs: "执行器没有给出产出清单引用",
    agent_runtime_executor_no_git_changes: "执行器没有产生任何 Git 变更",
    agent_runtime_executor_output_not_json: "执行器输出不是合法 JSON",
    agent_runtime_executor_output_too_large: "执行器输出超过上限被截断",
    approval_request_not_found: "这条审批请求找不到：可能已被撤回，也可能你没有它的可见权限",
    permission_request_not_found: "这条权限申请找不到：可能已被撤回，也可能你没有它的可见权限",
    high_risk_no_self_approval: "高风险变更不能由提出人自己批准：请另找一位有批准权限的人",
    control_plane_state_corrupt: "控制面状态文件内容已损坏，读不出来",
    control_plane_state_unrecognized: "控制面状态文件不像本系统写出来的（既没有版本号，也没有任何一个已知集合）",
    unsupported_state_schema_version: "控制面状态文件是更新的版本写的，这个构建认不出来",
    unsupported_project_shard_schema_version: "项目分片是更新的版本写的，这个构建认不出来",
    project_state_shard_corrupt: "项目分片文件内容已损坏，读不出来",
    project_state_shard_missing: "项目分片文件不见了",
    project_state_shard_identity_mismatch: "索引里记着的项目分片，内容对不上它自己的名字（多半被覆盖过）",
    project_state_shard_digest_mismatch: "项目分片的校验值对不上（内容被改过或写了一半）",
    project_state_shard_payload_digest_mismatch: "项目分片正文的校验值对不上",
    project_state_shard_payload_size_mismatch: "项目分片正文长度与记录的不一致",
    project_shard_safe_id_collision: "两个项目 id 落到了同一个分片文件名上",
    state_store_lock_timeout: "等状态写入锁超时（另一个写入方一直没放手）",
    state_unreadable: "状态读不出来，原因没归到已知的几类（看服务端日志）",
    runtime_dir_replaced: "运行目录已被换成另一份，不是启动时那个",
    runtime_dir_missing: "运行目录不见了",
    state_rebuilt_from_seed: "状态已按种子重建（原来那份没能读出来）",
    agent_runtime_executor_output_outside_allowlist: "执行器输出落在允许路径之外",
    agent_runtime_executor_push_remote_sha_mismatch: "执行器推送后的远端提交号与本地不一致",
    agent_runtime_executor_requires_clean_worktree: "执行器要求工作区干净后才能开始",
    agent_runtime_executor_uncommitted_changes_after_commit: "执行器提交之后仍有未提交的改动",
    agent_runtime_no_git_changes: "没有产生任何 Git 变更",
    agent_runtime_output_outside_allowlist: "产出落在允许路径之外",
    agent_runtime_push_remote_sha_mismatch: "推送后的远端提交号与本地不一致",
    agent_runtime_worker_requires_clean_worktree: "执行前要求工作区干净",
    artifact_manifest_binding_mismatch: "产出清单绑定的对象不匹配",
    artifact_manifest_contract_digest_mismatch: "产出清单里的契约摘要与实际契约不一致",
    artifact_manifest_has_no_output_beyond_itself: "产出清单里除了它自己没有任何产出",
    artifact_manifest_missing_output_refs: "产出清单缺少产出引用",
    artifact_manifest_must_be_git_trackable: "产出清单必须落在可被 Git 跟踪的路径上",
    artifact_manifest_not_changed_in_commit: "该提交没有改动产出清单",
    artifact_manifest_not_in_commit: "该提交里没有产出清单",
    artifact_manifest_not_json: "产出清单不是合法 JSON",
    artifact_manifest_outside_allowlist: "产出清单落在允许路径之外",
    artifact_output_ref_not_changed_in_commit: "该提交没有改动这个产出引用",
    artifact_output_ref_not_in_commit: "该提交里没有这个产出引用",
    artifact_output_ref_outside_allowlist: "产出引用落在允许路径之外",
    changed_paths_inside_repository_target_denylist: "改动路径落进了写入边界的禁止清单",
    changed_paths_outside_repository_target_allowlist: "改动路径落在写入边界的允许清单之外",
    changed_paths_outside_human_approved_plan: "改动路径超出人已定稿的执行方案所批准的范围",
    changed_paths_inside_human_forbidden_plan_paths: "改动路径踩到了人已定稿方案里划出的禁区",
    acceptance_check_evidence_missing: "人批准方案时看到的验收项没有对应证据",
    owned_paths_disjoint: "分支写到了自己不拥有的路径",
    resource_scopes_disjoint: "分支之间的资源范围重叠",
    independent_deliverables: "分支缺少可独立交付的目标",
    final_validation_available: "分支没有声明任何验收项",
    result_bundle_contract: "分支的结果包契约不完整",
    // 2026-08-22：这条不再只对并行报 —— 载体/隔离为 none 的单分支方案同样启动不了，
    // 阻塞项要在资格检查阶段就说出来（原先只对多分支报，人会白定稿一场）。文案跟着改准。
    runner_isolated: "方案没有指明执行载体或隔离方式",
    // 阻塞项的【尾段】此前一个中文都没有，人看到的是「（b_api · no_acceptance_checks）」。
    // 这些码由 topologyBlockerText 逐段走词表，查不到就原样打出来。
    no_acceptance_checks: "这个分支一条验收项都没写",
    missing_objective: "这个分支没写要达成什么",
    incomplete: "产出契约填得不全（至少要四项）",
    runner_or_isolation_none: "载体或隔离方式填的是「none」",
    runner_handle_uncertain: "执行载体状态未知，需要人工核对",
    blocking_derived_task_request: "被一条派生任务请求挡住",
    checkpoint_commit_has_no_changed_paths: "检查点提交没有任何改动路径",
    checkpoint_language_policy_digest_mismatch: "检查点的语言策略摘要与契约不一致",
    checkpoint_language_policy_digest_required: "检查点必须带上语言策略摘要",
    checkpoint_missing_git_evidence: "检查点缺少 Git 证据（提交/推送）",
    checkpoint_run_id_required: "检查点必须带上运行编号",
    checkpoint_schema_validation_failed: "检查点记录不符合机器契约",
    content_bundle_git_transfer_unsafe_repository_url: "内容包要拉的仓库地址不安全",
    content_bundle_git_transfer_escapes_session: "内容包要写的位置在本会话目录之外",
    content_bundle_git_transfer_unsafe_ref: "内容包指定的 git 引用不安全",
    dispatch_repository_url_not_cloneable: "派发没给出可克隆的仓库地址",
    agent_mcp_call_error: "执行方调用控制面时出错",
    agent_mcp_call_refused: "控制面拒绝了执行方的这次调用",
    agent_permission_request_not_created: "权限单没建起来（控制面没回单号）",
    agent_permission_request_timed_out: "等人处置权限单超时了",
    agent_permission_reassigned: "这件活已改派给别的节点",
    agent_permission_not_granted: "权限没批下来，这件活停住了",
    skill_file_digest_mismatch: "技能文件内容与控制面给的摘要对不上",
    dispatch_repository_url_unsafe_transport: "派发给的仓库地址用了不安全的 git 传输方式",
    local_repository_remote_mismatch: "机器上那份仓库的 remote 与派发指定的不是同一个",
    dispatch_model_decision_missing: "派发没说用哪个模型",
    artifact_manifest_path_escapes_repository: "产出清单路径指到仓库外面",
    dispatch_package_node_binding_mismatch: "这个派发包不是发给该节点的",
    dispatch_task_contract_digest_mismatch: "派发包与任务契约的摘要对不上",
    dispatch_skill_workset_binding_mismatch: "派发包与契约上的技能集不是同一个",
    agent_control_plane_retry_exhausted: "执行方反复与控制面冲突，重试用完了",
    agent_worktree_not_clean: "开工前执行方机器上的工作树就不干净",
    repository_path_outside_allowlist: "改到了这次派发不允许改的路径",
    repository_path_forbidden: "改到了这次派发禁止改的路径",
    checkpoint_task_contract_digest_required: "检查点没写它干的是哪份任务契约",
    checkpoint_task_contract_digest_mismatch: "检查点的任务契约摘要与契约不一致",
    commit_ref_not_found: "找不到该提交",
    commit_ref_target_mismatch: "提交指向的目标与写入边界不一致",
    commit_ref_tree_digest_mismatch: "提交的目录树摘要与声明不一致",
    confirmation_dispatch_node_mismatch: "该确认单不属于这个节点的派发",
    confirmation_task_group_mismatch: "该确认单不属于这个任务组",
    content_bundle_context_missing: "执行内容包缺少上下文",
    content_bundle_dispatch_not_active: "该派发不在进行中，无法取用内容包",
    project_repository_not_found: "这个项目里没有这个仓库 ID：可能刚被删掉或改了 ID，刷新项目设置再试",
    repository_credential_unreadable: "这个项目仓库的凭证密文解不开（多半是控制面换过凭证密钥 AIMAC_CREDENTIAL_KEY 或 credential.key 文件）：派发已拒绝认领，不会让 agent 拿主机上的凭证瞎试。到项目仓库设置里重新填一次账号密码或 API Key 即可",
    control_dispatch_not_active: "该派发不在进行中，无法下达控制命令",
    dispatch_package_incomplete: "派发包不完整",
    execution_topology_block_requires_derived_task_request_ref: "标记阻塞必须给出派生任务请求引用",
    execution_topology_blocker_not_found: "找不到该阻塞项",
    execution_topology_branch_not_found: "找不到该分支",
    execution_topology_cancel_requires_ref: "终止执行方案必须给出理由",
    execution_topology_downgrade_requires_reason: "降级执行方案必须给出理由",
    execution_topology_duplicate_branch_id: "分支编号重复",
    execution_topology_eligibility_blocked: "方案未通过启动前检查",
    execution_topology_external_runner_requires_grant_and_local_verification: "使用外部执行载体需要授权并在本地复验",
    execution_topology_merge_blocked_by_topology_blockers: "方案仍有阻塞项，不能合并",
    execution_topology_merge_requires_all_branches_reported: "所有分支都回报之后才能合并",
    execution_topology_merge_requires_final_validation_evidence: "合并必须给出最终验证证据",
    execution_topology_reconcile_requires_evidence: "和解方案必须给出证据",
    execution_topology_requires_runner_and_isolation: "方案必须指明执行载体与隔离方式",
    execution_topology_requires_work_item: "方案必须绑定任务",
    execution_topology_unblock_requires_resolved_ref: "解除阻塞必须给出处置引用",
    execution_topology_unknown_action: "未知的方案操作",
    finding_not_found: "找不到该发现项",
    invite_permission_not_delegable: "这条权限不可转授，不能通过邀请授予",
    join_token_consumed: "加入令牌已被使用",
    join_token_expired: "加入令牌已过期",
    join_token_invalid: "加入令牌无效",
    join_token_must_be_one_time: "加入令牌必须是一次性的",
    join_token_node_name_mismatch: "节点名与加入令牌登记的不一致",
    join_token_revoked: "加入令牌已被吊销",
    join_token_not_active: "加入令牌不处于可用状态",
    join_token_project_not_found: "加入令牌指向的项目不存在",
    checkpoint_claim_epoch_stale: "这份检查点来自该派发的上一次认领，当前持有者已换代 —— 它的提交属于上一次尝试，不能算作本轮成果",
    checkpoint_claim_epoch_required: "该派发被重新认领过，提交检查点必须带上持有的认领代次；若该节点的 agent 运行时早于 0.3.0（不发送代次），需重新执行入网安装命令升级",
    dispatch_fail_claim_epoch_required: "该派发被重新认领过，上报失败/阻塞必须带上持有的认领代次；若该节点的 agent 运行时早于 0.3.0（不发送代次），需重新执行入网安装命令升级",
    test_result_status_required: "提交测试结果必须显式给出状态（通过/失败/跳过/错误）—— 缺省不会被当作通过",
    approval_decision_required: "处理审批必须显式给出结论（批准/拒绝/取消）—— 缺省不会被当作批准",
    dispatch_fail_claim_epoch_stale: "这条失败上报来自该派发的上一次认领，当前持有者已换代 —— 不能用它把正在跑的这一轮标记为失败或阻塞",
    join_token_role_scope_mismatch: "申请的角色超出加入令牌的授权范围",
    agent_role_not_registered: "智能体角色不在已登记的执行角色里（照角色下拉里的填）",
    config_default_role_not_registered: "默认角色不在已登记的执行角色里（照角色下拉里的填）",
    mcp_token_invalid: "MCP 令牌不对或已失效",
    join_token_role_not_registered: "加入令牌的角色范围里有未登记的执行角色（这张票没有任何节点能用）",
    human_confirmation_action_required: "定稿决定必须明写动作（交 AI 再分析 / 定稿 / 打回），系统不会替你选一个",
    lease_already_active: "该资源已有生效租约",
    lease_fencing_token_mismatch: "租约栅栏令牌不匹配（可能有更晚的持有者）",
    lease_fencing_token_required: "必须带上租约栅栏令牌",
    lease_holder_mismatch: "租约持有者不是你",
    lease_holder_scope_mismatch: "租约持有者的作用域不匹配",
    lease_holder_session_settled: "租约持有者的会话已结束",
    lease_not_found: "找不到该租约",
    node_name_required: "必须填写节点名",
    permission_request_permission_not_delegable: "申请的权限不可转授",
    permission_request_resource_project_mismatch: "申请的资源不属于该项目",
    permission_request_resource_scope_unresolvable: "无法解析申请资源的作用域",
    permission_request_resource_type_not_allowed: "不允许对该类资源发起权限申请",
    permission_request_session_already_settled: "该会话已结束，权限申请不再有效",
    permission_request_session_scope_mismatch: "权限申请的会话作用域不匹配",
    permission_request_work_item_scope_mismatch: "权限申请的任务作用域不匹配",
    public_url_must_be_http_or_https: "对外地址必须是 http 或 https",
    push_ref_must_point_to_final_commit: "推送记录必须指向最终提交",
    push_ref_remote_repository_mismatch: "推送的远端仓库与写入边界不一致",
    push_ref_remote_sha_mismatch: "推送记录的远端提交号与实际不一致",
    push_ref_target_mismatch: "推送指向的目标与写入边界不一致",
    repository_output_target_already_pushed: "该写入边界已经推送过",
    repository_output_target_local_git_remote_disabled: "不允许使用本地 Git 远端",
    repository_output_target_not_found: "找不到该写入边界",
    repository_output_target_refs_must_match_single_session_target: "一个会话只能对应一个写入边界",
    repository_output_target_scope_mismatch: "写入边界的作用域不匹配",
    repository_output_target_unsafe_branch: "分支名不安全，已拒绝",
    repository_output_target_unsafe_remote: "远端地址不安全，已拒绝",
    role_skill_digest_mismatch: "角色技能摘要与登记的不一致",
    role_skill_not_found: "找不到该角色技能",
    role_skill_not_assigned_to_role: "该角色未获分配这个 Skill",
    role_skill_assignment_mismatch: "请求的 Skill 与集中分配不一致",
    selected_agent_role_skill_cannot_be_resolved: "选中的 Agent 角色 Skill 无法解析",
    role_skill_overlay_base_not_found: "找不到角色技能定制所基于的原始技能",
    role_skill_reference_ambiguous: "角色技能引用有歧义（匹配到多个）",
    project_owner_assignment_requires_project_creation: "项目负责人只能在创建项目时确定",
    project_owner_grant_immutable: "不能通过普通授权改写项目负责人",
    role_skill_registry_empty: "角色技能库为空",
    role_skill_role_not_registered: "该角色没有登记技能",
    role_skill_source_missing: "缺少角色技能来源",
    session_work_item_mismatch: "会话与任务不匹配",
    skill_source_unsafe_git_input: "技能源的 Git 参数不安全，已拒绝",
    skill_workset_not_found: "找不到该技能工作集",
    task_contract_missing_for_executor: "执行器缺少任务契约",
    // 规范里建模过的状态必须全部有中文：未命中时 t() 回退成原始英文键，并只往【浏览器控制台】
    // 打一条警告 —— 而真正的用户不会去看那里。于是人批准一个授权请求之后，中文界面上的徽标
    // 写着 "approved"。逐条写死的"某某必须本地化"断言查不出这类缺口，只有按 state-machines.yaml
    // 全量核对才会暴露（补这一批时是 32 个，覆盖 14 台机器）。
    approved: "已批准",
    classified: "已分类",
    routed_to_controller: "已转交管控方",
    external_capability_required: "需外部能力",
    external_capability_available: "外部能力可用",
    preauthorized_capability_bound: "已绑定预授权能力",
    external_capability_blocked: "外部能力被阻断",
    reassigned: "已改派",
    scope_reduced: "已缩小授权范围",
    retrying: "重试中",
    policy_precheck: "策略预检中",
    risk_classified: "已评定风险等级",
    wontfix: "不修复",
    compatibility_checking: "兼容性校验中",
    rebasing: "变基中",
    batch_ci_running: "批次 CI 运行中",
    batch_verified: "批次已验证",
    batch_ci_failed: "批次 CI 失败",
    verifying: "校验中",
    rollback_required: "需回滚",
    abandoned: "已放弃",
    replayed: "已重放",
    discarded: "已丢弃",
    joined: "已加入",
    persisted: "已留存",
    actor_authorization: "操作者授权",
    agent: "智能体",
    agent_identity: "智能体身份",
    ai_agent_cluster: "AI 智能体集群",
    ai_agent_session: "AI 智能体会话",
    ai_controller: "AI 总控",
    api_contract: "接口契约",
    artifact_path: "产出路径",
    authorization_integrity: "授权完整性",
    autonomous_service_agent: "自治服务智能体",
    capability_integrity: "能力完整性",
    checkpoint_integrity: "检查点完整性",
    data_model: "数据模型",
    db_schema: "数据库结构",
    db_table: "数据库表",
    decision_integrity: "决策完整性",
    design_token: "设计变量",
    deterministic_service_with_ai_planner: "确定性服务＋AI 规划",
    digest_integrity: "摘要完整性",
    dir_path: "目录路径",
    dynamic_context: "按上下文动态选",
    effect_integrity: "作用完整性",
    environment: "环境",
    error_code: "错误码",
    event_schema: "事件结构",
    field_presence: "字段存在性",
    file_path: "文件路径",
    filesystem: "文件系统",
    gateway: "服务网关",
    git: "Git",
    git_repo: "Git 仓库",
    git_worktree: "Git 工作树",
    human_finalizer: "真人定稿者",
    instruction_format: "指令格式",
    mcp_server: "MCP 服务端",
    mcp_tool: "MCP 工具",
    minor: "次要",
    permission_blocker: "权限受阻",
    permission_semantics: "权限语义",
    post_fix_validation: "修复后复验",
    quality_standard: "质量标准",
    reference_integrity: "引用完整性",
    repair_needed: "需要修复",
    remote_mcp: "远程 MCP",
    repeated_capacity_blocker: "反复的容量受阻",
    repeated_failure_fingerprint: "反复出现的失败指纹",
    repeated_model_selection_failure: "反复的选型失败",
    repeated_permission_blocker: "反复的权限受阻",
    repeated_review_finding: "反复的评审发现",
    repeated_spec_drift: "反复的规范漂移",
    repeated_test_flake: "反复的用例抖动",
    review_expand: "评审范围扩大",
    role_has_no_dedicated_model_policy: "该角色没有专属选型策略",
    shared_definition: "共享定义",
    spec_drift: "规范漂移",
    state_predicate: "状态判定",
    system_auditor: "系统审计员",
    system_console: "系统控制台",
    system_policy: "系统策略",
    system_service: "系统服务",
    test_failure: "用例失败",
    topology_replan: "执行方案重排",
    undecided: "尚未决定",
    user_console: "用户控制台",
    user_guidance: "人工指引",
    imported_pending_admin_activation: "已导入，等管理员激活",
    registered: "已登记",
    gc: "已回收",
    opened: "已开启",
    integrated: "已并入",
    batched: "已入批",
    published: "已发布",
    detected: "已发现",
    capability_registered: "能力已登记",
    quorum_collecting: "多方审批中（未达法定人数）",
    in_progress: "执行中",
    succeeded: "已成功",
    accepted: "已受理",
    rejected: "已拒绝",
    error: "错误",
    ok: "正常",
    attention: "需关注",
    active: "进行中",
    inactive: "已停用",
    disabled: "已停用",
    suspended: "已暂停",
    open: "打开",
    closed: "已关闭",
    aborted: "已中止",
    expired: "已过期",
    ready: "就绪",
    draft: "草稿",
    superseded: "已被取代",
    archived: "已归档",
    paused: "已暂停",
    unknown: "未知",

    /* ---------- 节点 / 运行时 ---------- */
    online: "在线",
    offline: "离线",
    degraded: "降级",
    draining: "撤出中",
    revoked: "已吊销",
    initializing: "初始化中",
    joining: "加入中",
    // 节点准入的三档：完全准入／受限准入／只读准入。原先后两档只写「受限」「只读」，在评审人这种只读身份眼里像在说他自己的权限。
    read_only: "只读准入",
    quarantine: "隔离中",
    quarantined: "已隔离",
    retired: "已退役",
    healthy: "健康",
    limited: "受限准入",
    full: "完全准入",
    admitted: "已准入",
    standby: "待命",
    monitoring: "监控中",
    stopped: "已停止",
    login_required: "待登录",
    initialized: "已初始化",
    validated: "已校验",
    configured: "已配置",
    current: "最新",
    stale: "已过时",
    stale_state: "状态过时",
    syncing: "同步中",
    indexed: "已索引",
    node_shutdown_completed: "节点已关停",

    /* ---------- 人工审核 / 人工指令 ---------- */
    answered: "已确认",
    consumed: "已采纳",
    acknowledged: "已知悉",
    applied: "已执行",
    decision: "决策",
    decision_request: "待决策请求",
    pause: "暂停执行",
    resume: "恢复执行",
    cancel: "取消任务",
    adjust_priority: "调整优先级",
    p0_safety: "P0：安全 / 资金 / 数据损坏 / 证据污染",
    unblock_many: "解阻塞：卡住多个下游单元",
    available_window: "窗口期：当前开着的限时外部窗口",
    current_condition: "当前正确性（默认档）",
    capability_data: "能力 / 数据 / 客户端工作",
    readiness_preflight: "只读 / 预检 / 就绪核对",
    formal_gate: "正式 / 性能 / 多实例门",
    add_requirement: "补充要求",
    free_text: "自由指令",

    /* ---------- 评审 / 验证 ---------- */
    review_requested: "待评审",
    review_passed: "评审通过",
    verified: "已验证",
    verification: "验证中",
    verification_ready: "待验证",
    reverify_required: "需复验",
    reverification: "复验中",
    changes_requested: "要求返工",
    coverage_satisfied: "覆盖达标",
    missing_git_evidence: "缺少 Git 证据",
    satisfied: "已满足",
    clear: "可关闭",
    passed: "已通过",
    waived: "已豁免",

    /* ---------- 任务组阶段（TaskGroup 状态机） ---------- */
    intake: "需求接入",
    discovery: "需求澄清",
    product_design: "产品设计",
    solution_design: "方案设计",
    ui_design: "界面设计",
    development: "开发中",
    global_development_review: "全局开发评审",
    repair: "修复中",
    integration: "集成中",
    release: "发布",
    online_quality: "线上质量",
    implementation: "实施中",
    governance_design: "治理设计",
    protocol: "协议设计",
    received: "已接收",

    /* ---------- 任务（WorkItem 状态机） ---------- */
    assigned: "已分派",
    checkpoint_submitted: "检查点已提交",
    code_complete: "编码完成",
    invalidated: "已作废",
    close_candidate: "待关闭",
    blocked_dependency: "依赖受阻",
    blocked_resource: "资源受阻",
    permission_required: "待授权",
    needs_decision: "待决策",
    reopened: "已重开",

    /* ---------- 会话（WorkSession 状态机） ---------- */
    starting: "启动中",
    waiting_room_event: "等待协作消息",
    waiting_dependency: "等待依赖",
    completed_objective: "目标达成",
    recycled: "已回收",

    /* ---------- 命令 / 控制通道 ---------- */
    created: "已创建",
    dispatched: "已派发",
    checkpointed: "已检查点",
    timed_out: "已超时",
    compensated: "已补偿",
    dlq: "死信",
    delivered: "已送达",
    acked: "已确认执行",
    pause_dispatch: "暂停派发",
    resume_dispatch: "恢复派发",
    cancel_dispatch: "取消派发",
    refresh_profile: "刷新档案",
    revoke: "吊销节点",
    shutdown: "关停节点",

    /* ---------- 执行事件类型 ---------- */
    dispatch_received: "已接收派发",
    skill_synced: "技能已同步",
    executor_started: "执行器已启动",
    executor_output: "执行输出",
    repository_changed: "仓库变更",
    git_committed: "已提交代码",
    git_pushed: "已推送代码",
    checkpoint_prepared: "检查点已准备",
    heartbeat: "心跳",
    drift_signal: "偏移信号",
    progress: "进度更新",
    dispatch_claimed: "派发已认领",
    dispatch_completed: "派发已完成",
    dispatch_failed: "派发已失败",

    /* ---------- 受阻 / 失败原因 ---------- */
    awaiting_human_confirmation: "等待人工确认",
    awaiting_plan_finalization: "等待人工定稿执行方案",
    plan_finalization_justification_required: "指定是否需要人工定稿执行方案时必须填写理由",
    governed_by_finalized_topology: "按人已定稿的执行方案推进",
    review_bundle_not_found: "评审包不存在",
    review_bundle_status_invalid: "评审包收尾状态无效",
    review_bundle_resolution_justification_required: "收尾评审包必须填写理由",
    system_upgrade_candidate_not_found: "系统升级候选项不存在",
    system_upgrade_candidate_status_invalid: "系统升级候选项状态无效",
    system_upgrade_candidate_justification_required: "处置系统升级候选项必须填写理由",
    upgrade_import_requires_package_ref: "导入外部升级包必须填写升级包引用",
    repeated_integration_conflict: "反复集成冲突",
    fixed_unverified: "已修复但无证据",
    blocked_external_incomplete: "外部阻塞但信息不全",
    evidence_refs_missing: "缺少证据引用",
    root_cause_owner_or_recovery_ref_missing: "缺少根因归属或恢复路径",
    human_confirmation_round_stale: "方案已在你查看后被修订，请重新查看最新一轮再决定",
    quality_gate_waive_requires_justification: "豁免质量门必须填写理由",
    quality_gate_not_found: "质量门不存在",
    review_plan_not_found: "评审计划不存在",
    review_plan_status_invalid: "评审计划收尾状态无效",
    review_plan_resolution_justification_required: "收尾评审计划必须填写理由",
    shared_definition_not_found: "共享定义契约不存在",
    shared_definition_status_invalid: "共享定义契约状态无效",
    rule_source_resolution_not_found: "规则来源分流记录不存在",
    rule_source_adoption_requires_human: "采纳为本项目规则必须由真人判定，AI 只能判不采纳",
    rule_source_status_invalid: "规则来源判定状态无效",
    finding_disposition_requires_human: "判为“不适用/已调整范围”属于不修即放行，必须由真人处置",
    test: "测试",
    lint: "静态检查",
    build: "构建",
    human_confirmation_cancelled_by_dispatch_failure: "待人工确认因执行失败被作废·已升级人工决策",
    awaiting_existing_checkpoint: "等待既有检查点",
    awaiting_analysis_output: "等待分析产出",
    role_drift_guard_blocked: "角色偏移守卫阻断",
    credential_required: "缺少凭证",
    agent_runtime_executor_required: "需智能体运行时执行器",
    awaiting_dependency: "等待依赖项完成",
    awaiting_decision: "等待人工决策",
    dependency_abandoned: "依赖已被放弃",
    shared_definition_not_active: "共享定义未生效",
    blocking_objects_present: "存在阻塞对象",
    existing_execution_active: "已有执行进行中",
    mixed_analysis_implementation_split: "分析与实现混合，已拆分",
    cell_processing_error: "任务处理异常（已隔离待决策）",
    task_group_recompute_error: "任务组重算异常（已隔离）",
    condition_window_deferred: "条件窗口未满足，已延后",
    independent_review: "独立评审中",
    gate_blocked: "门禁阻断",
    /* ---------- 人工指令：决策处置 ---------- */
    resolve_decision: "决策处置",
    work_item_decision_reopen: "任务重开",
    work_item_decision_abandon: "任务放弃",
    no_needs_decision_work_item: "无待决策任务",
    /* ---------- 准入决策 outcome / cellClass / 载体（§4.5） ---------- */
    deferred: "已延后",
    resource_queued: "资源排队中",
    awaiting_review: "等待独立评审",
    awaiting_checkpoint: "等待检查点",
    skipped_terminal: "已终态跳过",
    ready_now: "现在可执行",
    ready_after_resource_admission: "资源准入后可执行",
    pending_window: "等待条件窗口",
    pending_data_volume: "等待数据量达标",
    blocked_external: "外部阻断",
    fixed_verified: "已修复并验证",
    not_applicable: "不适用",
    scope_adjusted: "范围已调整",
    WorkItem: "任务", RoleDriftGuard: "角色漂移守卫", SharedDefinitionContract: "共享定义契约",
    RepositoryOutputTarget: "仓库产出目标", WorkSession: "工作会话", AgentDispatch: "智能体派发",
    Lease: "资源租约", Checkpoint: "检查点", ReviewBundle: "评审包", ExecutionTopology: "执行拓扑",
    IntegrationBatch: "集成批次",
    ReviewPlan: "评审计划", PermissionOrApprovalRequest: "授权/审批请求",
    CloseBarrierGate: "关闭门禁项", DerivedTaskRequest: "派生任务请求", CommandEffect: "命令副作用",
    HumanConfirmationRequest: "人工确认请求", HumanDirective: "人工指令", PermissionRequest: "授权请求",
    // 【关闭门的名字要写成没过时的样子】。这些名字只出现在一个地方：关闭门禁的「阻塞明细」，
    // 而那里【只列没过的门】。写成满足时的样子就自相矛盾 —— 屏幕上原本印着
    // 「无待处理评审包：受阻」「完成就绪已清：受阻」，人读到的是两句打架的话。
    // 26 道门里有 11 道是这样，按 core 的 gateFailures 全量核过（契约门在守这条）。
    all_quality_gates_passed: "还有质量门禁没通过", 
    no_pending_permissions: "还有授权请求没处理", 
    all_required_evidence_present: "证据齐备", 
    all_commands_terminal: "还有命令没到终态", 
    all_contracts_compatible: "还有契约不兼容", 
    no_active_temp_grants: "还有临时授权在生效", 
    runtime_issue_candidates_exported: "还有运行问题候选没导出", 
    no_blocking_derived_task_requests: "还有派生任务请求挡着", 
    no_pending_review_bundles: "还有评审包没收尾", 
    completion_readiness_clear: "完成就绪还没清干净", 
    no_active_role_drift_blockers: "还有角色漂移阻塞在生效",
    no_open_integration_batches: "还有集成批次没有完成",
    all_shared_definitions_active: "还有共享定义没生效", 
    no_open_execution_topology: "无未关闭执行拓扑", no_open_review_plan: "无未关闭评审计划",
    no_pending_review_bundle: "无待处理评审包", no_blocking_derived_task_request: "无阻塞派生任务",
    no_pending_external_review: "无待外部评审", no_active_role_drift_guard: "无活跃角色漂移守卫",
    shared_definitions_active: "共享定义生效中",
    repository_output_target_terminal: "仓库产出目标终态", all_required_outputs_present: "必需产出齐备",
    no_pending_permission_or_approval: "无待授权或审批", no_unreconciled_command_effect: "无未对账命令副作用",
    blocked_by_exact_dependency: "精确依赖阻断",
    diagnostic_only_no_pass: "仅诊断（不判通过）",
    defer_downstream: "延后至下游",
    short_subagent: "短时子智能体",
    reusable_top_level_lane: "复用顶层执行通道",
    new_top_level_lane: "新建顶层执行通道",
    integration_owner_direct: "集成负责人直接执行",
    control_pause_requested: "控制通道请求暂停",
    control_pause_rejected_by_node: "节点连续拒绝暂停，已停止重试（该 agent 可能仍在执行）",
    control_cancel_rejected_by_node: "节点连续拒绝取消，已停止重试（该 agent 可能仍在执行）",
    assigned_node_stop_control_failed_retries_exhausted: "节点停止控制连续失败，重试已用尽",
    control_resume_requested: "控制通道请求恢复",
    control_cancel_requested: "控制通道请求取消",
    claim_expired_requeued: "认领超时重新入队",
    revocation_ack_timeout_requeued: "吊销确认超时重新入队",
    shutdown_ack_timeout_requeued: "停机确认超时重新入队",
    paused_node_dead_requeued: "暂停节点失联后重新入队",
    assigned_node_stop_control_failed_retry_queued: "节点停止控制失败，重试入队",
    assigned_node_revocation_ack_requeued: "节点吊销确认后重新入队",
    assigned_node_shutdown_ack_requeued: "节点关停确认后重新入队",
    assigned_node_revocation_pending_stop: "节点吊销待停止",
    assigned_node_shutdown_pending_stop: "节点关停待停止",
    permission_request_approved: "权限申请已批准",
    local_mcp_policy_eval: "本地 MCP 策略判定",
    model_selection_rejected: "模型选择被拒绝",
    human_directive_cancel: "人工指令取消",
    human_directive: "人工指令",
    active_paused_by_control: "已被控制暂停",
    human_directive_pause: "人工暂停",
    active_paused_by_freeze: "已被冻结暂停",
    independent_review_changes_requested: "独立评审要求返工",
    independent_review_backfill_failed: "补评审未通过待决策",
    mcp_session_paused: "MCP 会话已暂停",
    remote_mcp_principal_grant: "远程 MCP 主体授权",
    permission_request_denied: "权限被拒待决策",
    permission_request_pending: "等待授权审批（已超时挂起）",
    /* ---------- 人工定稿闸门 ---------- */
    work_item_verification: "任务验收定稿",
    task_group_close: "任务组关闭定稿",
    plan_topology: "执行方案／拓扑选择",
    task_split: "任务拆分方案",
    rule_change: "规则／配置变更",
    runtime_execution: "运行时执行确认",
    review_passed_awaiting_human_confirmation: "互审通过·待人工定稿",
    human_verification_rejected: "人工验收未通过·待返工决策",
    execution_halted: "执行已被治理动作挡住（任务组暂停或组织停用）",
    human_confirmation_expired: "人工确认已超时·待人工决策",
    awaiting_human_split_confirmation: "等待人工确认拆分方案",
    cell_held_for_human_plan_confirmation: "方案待人工定稿，暂缓执行",
    execution_topology_requires_human_plan_confirmation: "执行方案需人工定稿后才能启动",
    task_group_close_requires_human_actor: "关闭任务组必须由真人账号执行",
    dispatch_halted_by_human_control: "这个派发已被真人叫停，先恢复再上报执行结果",
    dispatch_not_assigned_to_node: "这个派发不在该节点手上（已被回收或分给了别的节点）：刷新一下再看它现在归谁",
    agent_control_command_ack_status_unknown: "认不出的控制命令回执状态",
    dispatch_fail_status_unknown: "认不出的上报状态（blocked 可恢复，cancelled/failed 是终态）",
    ai_confirmation_assessment_unknown: "认不出的 AI 评估结论",
    human_confirmation_expired_needs_decision: "人工确认超时·已升级为人工决策",
    human_confirmation_requires_human_actor: "核心决策只能由真人账号确认，机器主体无权定稿",
    human_confirmation_decision_forbidden_for_machine_principal: "机器主体不得代为定稿，请在控制台人工确认窗口操作",
    system_upgrade_external_import_forbidden_for_machine_principal: "机器主体不得导入系统外升级结果，请由系统管理员在后台操作",
    human_confirmation_subject_changed: "该方案在你确认前已被改动，请重新查看后再定稿",
    human_finalization_subject_missing: "定稿对象已不存在，无法定稿",
    human_confirmation_expected_round_required: "核心决策定稿必须携带轮次令牌",
    human_confirmation_not_awaiting_ai_analysis: "当前不处于等待 AI 再分析的状态",
    human_finalized_decision_diverged: "该方案已由人工定稿，AI 变更与定稿内容不一致，已拦下并需重新人工确认",
    propose: "提出方案",
    analysis: "AI 再分析",
    // 协商记录里人这一侧的动作名直接进那一行：finalize/reject 都有中文，唯独 revise 漏了，
    // 于是"提交修改意见"那一轮在定稿页上显示成英文 revise —— 恰恰是人回看自己说过什么的地方。
    revise: "提交修改意见",
    finalize: "定稿",
    ran: "已推进",
    unchanged: "已运行，无变化",
    cycle_error: "周期出错",
    state_unavailable: "读不到状态",
    no_open_task_group: "没有进行中的任务组",
    execution_failed_repeatedly: "连续多次执行失败，已停止自动重派",
    idle: "空闲",
    busy: "忙碌",
    confirmed: "已确认",
    orphaned: "已失去归属",
    all_required_work_closed: "还有任务没有收口",
    all_findings_terminal: "还有发现项没有处置",
    all_changes_integrated: "还有改动没有合入",
    no_pending_approvals: "还有审批请求没有处理",
    all_command_effects_terminal: "还有命令效果没有和解",
    no_active_dlq: "死信队列里还有条目",
    all_leases_terminal: "还有写锁没有释放",
    artifacts_verified: "还有产物没有核验",
    rules_candidates_processed: "还有规则候选没有判定",
    no_open_execution_topologies: "还有执行方案没有了结",
    all_review_plans_closed: "还有评审计划没有收尾",
    all_rule_sources_resolved: "还有规则来源没有判定",
    no_pending_human_confirmations: "还有人工确认单等着定稿",
    no_pending_human_directives: "还有人工指令没有确认被消费",
    all_repository_output_targets_terminal: "还有写入目标没有终结",
    reject: "打回返工",
    plan_finalization_required: "已要求人工定稿",
    plan_finalization_cleared: "已取消人工定稿要求",
    agree: "认可该方案",
    concerns: "存在疑虑",
    better_alternative: "有更优方式",
    incorrect: "认为不正确",
    ai_analysis_summary_required: "AI 再分析必须给出结论说明",
    permission_request_approved_requeued: "授权通过，已重新排队",

    /* ---------- 账号 / 组织 ---------- */
    invited: "已邀请",
    locked: "已锁定",
    system_admin: "系统管理员",
    org_admin: "组织管理员",
    user_account: "组织成员",
    service_account: "服务账号",
    authenticated: "已登录",
    invite_token: "邀请令牌",
    bootstrap_token: "初始化令牌",
    service_token: "服务令牌",
    password: "密码",

    /* ---------- 角色 ---------- */
    orchestrator: "总控编排",
    "agent-runtime": "通用任务执行",
    "policy-engine": "策略引擎",
    "room-broker": "协作室代理",
    "control-plane": "控制面",
    "agent-gateway": "Agent 网关",
    "identity-service": "身份服务",
    "instruction-optimizer": "指令优化器",
    "permission-gateway": "权限网关",
    "mcp-proxy": "MCP 代理",
    "decision-center": "决策中心",
    "command-bus": "命令总线",
    "auth-service": "认证服务",
    "bootstrap": "初始化",
    "repository-router": "仓库路由",
    "skill-registry": "技能注册表",
    // MCP 的 account_invite 写的就是这个角色（HTTP 那条路默认 member），而成员管理页按 t(role)
    // 渲染角色列 —— 漏了它，经 MCP 邀请进来的成员在中文界面上顶着一个英文角色名。
    project_member: "项目成员",
    reviewer: "评审员",
    qa: "质量保障",
    security: "安全",
    monitor: "监控",
    scheduler: "调度器",
    system_owner: "系统负责人",
    workspace_owner: "工作区负责人",
    project_owner: "项目负责人",
    project_admin: "项目管理员",
    task_group_owner: "任务组负责人",
    agent_operator: "智能体操作员",
    viewer: "观察者",
    member: "成员",
    service_agent_runtime: "运行时服务角色",
    "ui-console-service": "控制台服务",
    "human-operator": "人工操作员",
    "human-reviewer": "人工审核员",
    auto: "自动加入",
    user: "手动添加",
    inherited: "继承项目",
    customized: "已自定义",

    /* ---------- 授权 / 令牌 ---------- */
    requested: "已申请",
    policy_checked: "策略已校验",
    issued: "已签发",
    used: "已使用",
    exhausted: "已用尽",
    granted: "已授权",
    denied: "已驳回",
    // 登录失败不是"被谁驳回了"，台账上要写清是凭据不对（此前复用 denied，
    // 真实台账读出来是"登录 Account:x 已驳回"，看着像有人审批过）。
    credentials_invalid: "凭据不正确",
    allowed: "已允许",
    evaluating: "评估中",
    grant_issued: "授权已签发",

    /* ---------- 仓库产出 ---------- */
    candidate: "候选",
    selected: "已选定",
    lease_bound: "租约已绑定",
    writing: "写入中",
    committed: "已提交",
    pushed: "已推送",
    merged: "已合并",
    merge_ready: "待合并",
    merge_conflict: "合并冲突",
    rolled_back: "已回滚",
    prepared: "已准备",
    submitted: "已提交",
    uploaded: "已上传",
    redacted: "已脱敏",
    released: "已释放",
    renewing: "续期中",

    /* ---------- 模型 / 调度 ---------- */
    available: "可用",
    unavailable: "不可用",
    quota_limited: "配额受限",
    availability_unavailable: "模型不可用",
    availability_quota_limited: "模型配额受限",
    observed: "已观测",
    probed: "已探测",
    discovered: "已发现",
    parsed: "已解析",
    overridden: "已覆盖",
    proposed: "已提议",
    capacity_checked: "容量已校验",
    new_session_selected: "选定新会话",
    subagent_selected: "选定子智能体",
    new_session: "新会话",
    subagent: "子智能体",
    balanced: "均衡",
    deep: "深度",
    auto_best: "自动最优",
    auto_fast: "自动快速",
    cost_aware: "成本优先",
    model_executor: "模型执行器",

    /* ---------- 指令 / 共享定义 ---------- */
    drafted: "已起草",
    compacted: "已压缩",
    cache_indexed: "已入缓存索引",
    owner_assigned: "已指定归属",
    reviewing: "评审中",
    change_requested: "要求变更",
    conflicted: "冲突",
    assembled: "已组装",
    strengthened: "已强化",
    assembling: "组装中",
    reference_only: "仅供参考",
    resolved: "已解决",

    /* ---------- 治理 / 监控 ---------- */
    bound: "已绑定",
    drift_detected: "检测到偏移",
    correction_required: "需纠偏",
    corrected: "已纠偏",
    monitor_attention: "监控告警",
    clustered: "已聚类",
    candidate_created: "候选已创建",
    suppressed: "已抑制",
    sample_recorded: "样本已记录",
    unreconciled: "未对账",
    non_terminal: "未到终态",
    not_active: "未生效",
    collecting: "收集中",
    evaluated: "已评估",
    exported_for_external_maintenance: "已导出外部维护",
    external_maintenance_required: "需外部维护",
    imported_after_external_upgrade: "外部升级后已导入",
    dismissed: "已忽略",
    reported: "已上报",
    triaged: "已分诊",
    duplicate: "重复",
    root_cause_confirmed: "根因已确认",
    fixed: "已修复",
    reverified: "已复验",
    raised: "已产生",
    routed: "已路由",
    appended: "已追加",
    sealed: "已封存",
    absorbed: "已吸收",
    backlog: "已入待办",
    downgraded: "已降级",
    needs_reconcile: "需对账",
    planned: "已规划",
    eligibility_checked: "资格已校验",
    integrating: "集成中",

    /* ---------- 系统服务 / 管理面 ---------- */
    system_management: "系统管理台",
    user_management: "用户管理台",
    runtime_json: "本地运行态文件",
    postgresql: "PostgreSQL",
    production: "生产",
    kind_major: "大项",
    kind_minor: "小项",
    system: "系统",
    project: "项目",
    task_group: "任务组",
    organization: "组织",
    control: "控制",
    policy: "策略",
    quality: "质量",
    runtime: "运行时",
    review: "评审",
    ui: "界面",

    /* ---------- 模型供应商（品牌名） ---------- */
    anthropic: "Anthropic",
    openai: "OpenAI",
    google: "Google",
    vertex_ai: "Google Vertex AI",
    azure_openai: "Azure OpenAI",
    aws_bedrock: "AWS Bedrock",
    meta: "Meta",
    mistral: "Mistral",
    deepseek: "DeepSeek",
    qwen: "通义千问",
    baidu: "百度文心",
    tencent: "腾讯混元",
    zhipu: "智谱",
    moonshot: "月之暗面",
    xai: "xAI",
    openrouter: "OpenRouter",
    ollama: "Ollama",
    vllm: "vLLM",
    custom: "自定义",

    /* ---------- 阻塞 / 严重度 ---------- */
    S0: "阻断",
    S1: "严重",
    S2: "一般",
    S3: "轻微",
    critical: "阻断",
    warning: "警告",
    info: "提示",
    major: "严重",
    normal: "一般",

    /* ---------- 错误码（api 报错中文化） ---------- */
    auth_required: "请先登录",
    invalid_credentials: "账号或登录令牌不正确",
    too_many_login_attempts: "登录尝试过于频繁，请稍后再试",
    permission_denied: "无权执行该操作",
    policy_denied: "策略校验未通过，权限不足",
    principal_not_allowed_for_action: "当前身份不允许执行该操作",
    project_owner_assignment_denied: "无权指定项目负责人",
    idempotency_key_required: "这次写请求没有带幂等键，因此没有执行：写接口要求每一笔都带 Idempotency-Key（同一笔重发时沿用同一个键，这样重发不会做两遍）",
    idempotency_key_reuse_conflict: "这个幂等键上一次配的是【另一笔】请求（人、动作或内容不同），因此这次没有执行：原样重发还是同样的错，要么换一个新键，要么先确认上一次那笔的结果",
    role_drift_guard_not_clear: "角色偏移守卫未清除，暂不能执行",
    state_write_conflict: "另一处同时改了同一份状态，你这次的改动没有写进去（是整笔被拒，不会只写一半）：刷新看一眼当前状态，再决定要不要重做",
    state_conflict_not_recovered: "状态冲突未能自动恢复，请刷新后重试",
    server_error: "服务器内部错误",
    api_not_found: "接口不存在",
    invalid_request_url: "请求地址不合法",

    unsafe_grant_permissions: "包含不允许下放的权限",
    grant_permission_not_delegable: "存在不可下放的权限",
    grant_role_has_no_permission_template: "这个作用域上没有该角色的权限模板：请改用支持的角色，或直接指定权限",
    cross_org_member_not_allowed: "不允许跨组织添加成员",
    account_role_unknown: "账号角色不在词表里（授权模板的角色名不是账号角色）",
    permission_unknown: "权限不在词表里（没有任何判定会要它，发出去也什么都打不开）",
    task_group_role_not_registered: "任务组的执行角色不在登记册里（拼错的角色要等到派发才会炸，这里先拒）",
    skill_source_stale: "技能源同步不上，agent 用的还是上一次同步下来的技能",
    agent_node_heartbeat_overdue: "节点心跳已超时、尚未被标成离线",
    role_skill_overlay_patch_unknown_keys: "角色技能叠加的 patch 里有认不出的键",
    project_invite_cannot_grant_system_account_or_permission: "项目邀请不能授予系统级账号或权限",

    wip_capacity_reached: "在制品已达上限",

    org_quota_exceeded: "组织配额已超限",
    org_quota_unreadable: "组织配额或用量不是个数字，读不出来（已按超限处理）",
    // 请求体读取阶段的四个码：它们在任何路由之前就产生，此前一个中文都没有 ——
    // 粘一大段规则超了 2MB、上传中途网络抖一下，界面上就是一串英文码。
    request_body_too_large: "提交的内容超过 2MB 上限，请分批提交或精简内容",
    request_body_invalid_json: "提交的内容不是合法 JSON（多半是集成方拼错了报文）",
    request_aborted: "请求在传输中被中断（网络断了或页面被关掉），没有任何改动生效",
    request_stream_error: "读取提交内容时连接出错，没有任何改动生效",
    organization_not_found: "未找到组织",
    organization_suspended: "组织已停用",
    organization_required: "缺少组织信息",
    org_member_not_found: "未找到组织成员",

    execution_topology_branch_status_unknown: "分支状态认不出来：只能是 reported（已上报）、failed（失败）、rejected（被拒）、blocked（受阻）。这次没有记 —— 认不出的状态若按「已上报」处理，一条真失败的分支会被当成做完了",
    rule_source_settlement_justification_too_long: "你写的处置依据超出了上限：这一条没有保存 —— 悄悄截断会让台账上记的与你写的不一致，而这条处置本来就是靠依据留痕的。请精简后重新提交（报文里给出了超出多少字）",
    shared_definition_resolution_justification_too_long: "你写的处置依据超出了上限：这一条没有保存 —— 悄悄截断会让台账上记的与你写的不一致。请精简后重新提交（报文里给出了超出多少字）",
    human_confirmation_input_too_long: "你写的定稿意见超出了上限：这一条没有保存 —— 悄悄截断会让台账上记的与你写的不一致，而人工定稿的全部意义就是「这句话是这个人说的」。请精简后重新提交（报文里给出了超出多少字）",
    rule_status_unknown: "规则状态认不出来：只能是 active（生效）、draft（草稿）、disabled（停用）。这次整批都没保存 —— 认不出的状态若按「生效」处理，你以为关掉的规则其实还在生效，所以宁可拒绝",
    rule_content_too_long: "规则内容过长（上限 8192 字），请精简后再保存",
    rule_title_too_long: "规则标题过长（上限 256 字）",
    rule_id_too_long: "规则标识过长（上限 128 字）",
    too_many_rules: "规则数量超出上限（最多 200 条）",

    human_confirmation_not_pending: "该人工确认已被处理，无法重复决策",
    human_confirmation_option_invalid: "所选确认选项无效",
    human_confirmation_options_required: "缺少可选确认项",
    human_confirmation_question_required: "缺少确认问题描述",
    human_directive_instruction_required: "缺少指令内容",
    human_directive_project_required: "缺少项目信息",

    project_not_found: "未找到项目",
    access_grant_not_found: "未找到访问授权",
    cross_org_grant_not_allowed: "不允许跨组织授权",
    project_id_required: "缺少项目 ID",
    unsupported_task_group_control_action: "不支持的任务组控制动作",
    task_group_id_conflict: "任务组 ID 冲突",
    work_item_id_conflict: "任务 ID 冲突",
    pinned_model_not_registered: "指定的模型不在模型能力注册表里：请在「系统设置 → 模型能力注册」核对可用模型，或改选「自动」",
    work_session_not_found: "未找到工作会话",
    dispatch_not_found: "未找到派发",
    owner_account_not_found: "未找到负责人账号",
    account_not_found: "未找到账号",
    account_email_already_registered: "该邮箱已被注册",
    // 自由文本上限：拒绝而不是静默截断（存下的与人写的不一致更难查）。
    work_item_requirements_too_many_items: "机器可执行要求给得太多了",
    work_item_requirements_item_too_long: "有一条机器可执行要求太长了",
    agent_node_name_too_long: "节点名太长了",
    project_name_too_long: "项目名太长了",
    task_group_name_too_long: "任务组名称太长了",
    task_group_objective_too_long: "任务组目标太长了",
    work_item_title_too_long: "任务标题太长了",
    organization_admin_email_required: "创建组织必须指定初始组织管理员的邮箱（它是这个人的登录身份）",
    agent_not_found: "未找到智能体档案",
    agent_node_not_found: "未找到 agent 节点",
    agent_node_auth_required: "agent 节点需要认证",
    agent_join_token_not_found: "未找到加入令牌",
    human_confirmation_not_found: "未找到人工确认请求",
    human_confirmation_input_required_for_none: "选择“不选择（自定义输入）”时必须填写确认内容",

    password_too_short: "新密码长度不足（至少 8 位）",
    current_password_incorrect: "当前密码不正确",

    server_side_agent_execution_forbidden: "禁止在服务端直接执行智能体任务，请由智能体运行时认领派发",
    repository_output_target_missing: "缺少仓库产出目标",
    repository_output_target_must_use_git_trackable_paths: "仓库产出路径必须是可被 Git 跟踪的路径",
    agent_dispatch_requires_selected_model_decision: "派发前需要已选定的模型决策",
    skill_source_not_found: "未找到技能源",

    /* ---------- 审计动作 / 操作类型 ---------- */
    execute: "执行",
    recorded_without_task_group: "无任务组记录",
    command_succeeded: "命令成功",
    policy_decision_denied: "策略拒绝",
    auth_login: "登录",
    auth_logout: "退出登录",
    auth_change_password: "修改密码",
    account_invite: "邀请账号",
    access_grant_create: "创建访问授权",
    access_grant_revoke: "撤销访问授权",
    agent_activation_update: "更新智能体启用状态",
    agent_create: "创建智能体",
    agent_node_register: "注册运行节点",
    agent_join_token_create: "创建加入令牌",
    agent_join_token_revoke: "吊销加入令牌",
    // 下面这些不是字面量写出来的，是 `task_group_${action}` / `agent_control_${commandType}` /
    // `dispatch_${reportedStatus}` 按各自的闭集拼出来的 —— 一样要显示在审计页那一列上。
    task_group_recompute_readiness: "重算任务组完成度",
    task_group_request_review: "请求评审任务组",
    task_group_finish_review: "结束任务组评审",
    task_group_rebound_drift: "标记任务组偏航",
    task_group_cancel: "取消任务组",
    task_group_abort: "中止任务组",
    agent_control_refresh_profile: "下发刷新节点画像",
    agent_control_pause_dispatch: "下发暂停派发",
    agent_control_cancel_dispatch: "下发取消派发",
    agent_control_resume_dispatch: "下发恢复派发",
    agent_control_shutdown: "下发关停节点",
    agent_control_revoke: "下发吊销节点",
    dispatch_blocked: "派发被阻塞",
    dispatch_cancelled: "派发被取消",
    agent_node_revoke: "吊销 agent 节点",
    approval_request_create: "发起审批",
    approval_resolve: "裁决审批",
    artifact_register: "登记产出物",
    checkpoint_rejected: "驳回检查点",
    contract_publish: "发布契约",
    derived_task_classify: "判定衍生任务归类",
    execution_topology_plan: "规划执行拓扑",
    execution_topology_advance: "推进执行拓扑",
    integration_batch_create: "创建集成批次",
    integration_batch_advance: "推进集成批次",
    integration_batch_already_terminal: "集成批次已经结束",
    integration_batch_expected_queued_got_rebasing: "集成批次当前正在 rebase，不能重复开始",
    integration_batch_not_found: "找不到集成批次",
    integration_batch_requires_baseline_commit: "集成批次缺少基线提交",
    integration_batch_requires_change_set_refs: "集成批次缺少变更集合引用",
    integration_batch_schema_validation_failed: "集成批次记录不符合规范",
    integration_batch_task_group_and_changes_required: "创建集成批次需要任务组和变更集合",
    integration_batch_transition_evidence_missing: "推进集成批次缺少必要证据",
    integration_batch_unknown_action: "认不出的集成批次动作",
    finding_submit: "提交问题项",
    finding_resolve: "处置问题项",
    lease_claim: "取得租约",
    lease_release: "释放租约",
    org_member_invite_reissue: "重发成员邀请",
    permission_request_submit: "提交权限申请",
    permission_resolve: "裁决权限申请",
    policy_decision_eval: "评估策略决策",
    project_archive: "归档项目",
    quality_gate_waive: "豁免质量门",
    review_plan_create: "创建互审计划",
    review_plan_resolve: "裁决互审计划",
    review_bundle_register: "登记互审包",
    review_bundle_resolve: "裁决互审包",
    room_send: "发送房间消息",
    rule_source_resolve: "解析规则来源",
    rule_source_settle: "定稿规则来源",
    shared_definition_resolve: "裁决共享定义",
    system_upgrade_candidate_resolve: "裁决系统升级候选",
    system_upgrade_external_import: "导入外部升级包",
    work_assign: "指派任务",
    work_item_plan_finalization_set: "标记任务方案定稿",
    agent_runtime_worker_run: "运行智能体工作循环",
    checkpoint_submit: "提交检查点",
    human_confirmation_request: "发起人工确认",
    human_confirmation_decide: "决策人工确认",
    human_directive_create: "下达人工指令",
    instruction_envelope_create: "创建指令封套",
    model_capability_register: "注册模型能力",
    model_selection_decide: "决策模型选择",
    orchestrator_run: "运行编排循环",
    org_create: "创建组织",
    org_member_create: "创建组织成员",
    org_member_permissions_update: "更新成员权限",
    org_member_status_update: "更新成员状态",
    account_retire: "注销账号（不可恢复）",
    org_project_create: "创建组织项目",
    org_quota_update: "更新组织配额",
    org_status_update: "更新组织状态",
    project_config_update: "更新项目配置",
    project_repository_connection_test: "测试项目仓库连接",
    project_create: "创建项目",
    project_member_grant: "授予项目成员",
    project_member_revoke: "移出项目成员",
    project_owner_role_immutable: "项目负责人角色不能通过普通成员授权修改",
    project_owner_cannot_be_removed: "项目负责人不能被移出项目",
    project_member_not_found: "项目中找不到该成员",
    repository_output_target_select: "选择仓库产出目标",
    role_skill_overlay_create: "创建角色技能覆盖",
    runtime_issue_collect: "采集运行时问题",
    session_placement_decide: "决策会话放置",
    shared_definition_contract_create: "创建共享定义契约",
    skill_source_sync: "同步技能源",
    skill_source_retire: "退役技能源",
    transition_rejected_in_warn_mode: "宽松模式下放行了一次本该被拒的状态迁移",
    mcp_tool_call: "MCP 工具调用",
    skill_source_already_retired: "技能源已经退役过了",
    plan_finalization_requirement_required: "必须明确指定是否需要人工定稿方案",
    human_directive_type_unknown: "认不出这个指令类型",
    human_directive_resolution_unknown: "认不出这个处置方式",
    dlq_entry_resolve: "处置死信条目",
    dlq_entry_resolution_required: "处置死信必须选择「丢弃」或「重放」",
    dlq_entry_justification_required: "处置死信必须写明理由",
    dlq_entry_not_found: "找不到该死信条目",
    dlq_entry_already_resolved: "该死信条目已被处置（可能另一个人刚处理完）",
    human_directive_priority_class_required: "调整优先级必须选一个档位（或指令里写明 p0/urgent 等关键词）",
    human_directive_task_group_required_for_work_item: "指定任务时必须同时指定所属任务组",
    human_directive_work_item_not_supported: "该指令只能作用于整个任务组",
    human_directive_work_item_not_found: "目标任务不属于指定任务组",
    human_directive_work_item_already_terminal: "目标任务已经结束，不能再下达任务级执行指令",
    finding_status_unknown: "认不出这个发现状态",
    review_plan_id_conflict: "这个评审计划 id 已经被创建过了",
    rule_source_resolution_id_conflict: "这个规则来源判定 id 已经被创建过了",
    upgrade_import_id_conflict: "这个外部升级导入编号已经存在",
    skill_workset_response_not_json: "技能集下载回的不是 JSON",
    expires_at_invalid: "到期时间解析不了",
    ttl_seconds_invalid: "有效期必须是数",
    control_command_expires_at_invalid: "控制命令的到期时间解析不了",
    guarded_action_verdict_required: "记受守卫动作必须显式给出允许或拒绝",
    policy_decision_verdict_required: "记策略决策必须显式给出放行或拒绝",
    permission_decision_required: "处理授权申请必须显式给出批准或驳回",
    artifact_id_conflict: "这个产出 id 已经被登记过了",
    approval_request_action_required: "审批单必须写明批的是哪个动作",
    agent_status_unknown: "认不出这个智能体状态",
    agent_trust_score_invalid: "信任分必须是 0 到 1 之间的数",
    task_group_status_unknown: "认不出这个任务组状态",
    work_item_status_unknown: "认不出这个任务状态",
    human_directive_instruction_too_long: "指令内容超出长度上限",
    quality_gate_waive_justification_too_long: "豁免理由超出长度上限",
    review_bundle_justification_too_long: "处置理由超出长度上限",
    system_upgrade_candidate_justification_too_long: "处置理由超出长度上限（升级候选）",
    upgrade_import_package_ref_too_long: "外部升级包引用超出长度上限",
    plan_finalization_justification_too_long: "理由超出长度上限（方案定稿要求）",
    review_plan_justification_too_long: "处置理由超出长度上限（评审计划）",
    task_group_close_barrier_compute: "计算收尾门禁",
    task_group_config_reset: "重置任务组配置",
    task_group_config_update: "更新任务组配置",
    task_group_create: "创建任务组",
    task_group_control: "任务组控制",
    task_group_language_policy_update: "更新语言策略",
    task_group_work_item_create: "创建任务",
    task_group_agent_control_command_create: "下发节点控制指令",
    task_group_pause: "暂停任务组",
    task_group_resume: "恢复任务组",
    task_group_priority_adjusted: "调整优先级",
    task_group_guidance_appended: "追加执行指引",
    task_group_requirement_appended: "追加需求",
    work_item_priority_adjusted: "调整指定任务优先级",
    work_item_guidance_appended: "追加指定任务执行指引",
    task_group_cancel_pending_dispatches: "取消待派发任务",
    work_item_requirement_appended: "追加任务需求",
    bootstrap_init: "重新初始化运行态",
    runtime_initialized: "运行态已初始化",

    /* ---------- 模型能力标签 ---------- */
    planning: "规划",
    architecture: "架构",
    deep_reasoning: "深度推理",
    long_context: "长上下文",
    fast_execution: "快速执行",
    coding: "编码",

    /* ---------- 共享定义类型 ---------- */
    status_semantics: "状态语义",
    semantic_contract: "语义契约",
    terminology: "术语定义",

    /* ---------- 会话放置方式补充 ---------- */

    /* ---------- 失败原因里 "code:detail" 的那一段前缀（由 explainCoded 取用） ---------- */
    git_command_failed: "git 命令失败",
    git_auth_failed: "git 认证被远端拒绝：仓库凭证不对、已过期或没有这个仓库的权限（说明里写着用的是项目仓库配置投递的凭证还是 agent 主机自己的）",
    git_repository_not_found: "远端说没有这个仓库：地址写错、仓库被删或改名，或这份凭证看不到它",
    git_remote_unreachable: "够不着 git 远端：域名解析不了、端口不通或网络被拦",
    /* ---------- agent 运行时抛出的失败原因（同样经派发失败原因显示给人） ---------- */
    content_bundle_sync_failed: "内容包同步失败，没有同步到规则就不允许执行",
    content_bundle_manifest_mismatch: "内容包整包摘要对不上",
    content_bundle_digest_mismatch: "内容包里某个文件的摘要对不上",
    content_bundle_path_escapes_session: "内容包里有路径指向会话目录之外",
    checkpoint_replay_recover_required: "检查点重放需要人工恢复",
    executor_produced_no_changes: "执行器没有产出任何仓库改动",
    executor_produced_no_output: "执行器没有产出任务输出",
    push_verification_failed: "推送校验没过",
    executor_exited_nonzero: "执行器非正常退出",
    executor_model_id_required: "执行器缺少模型标识",
    executor_not_installed: "这台节点上没有可用的执行器",
    skill_workset_digest_mismatch: "技能集摘要对不上",
    skill_workset_path_escape: "技能集里有越界路径",
    skill_workset_download_failed: "技能集下载失败",
    content_bundle_git_transfer_failed: "内容包大文件拉取失败",
    agent_runtime_executor_failed: "执行器没能跑完",
    agent_runtime_executor_undeclared_changes: "执行器改了没申报的文件",
    agent_runtime_executor_declared_unchanged_paths: "执行器申报了却没改的文件",
    pinned_commit_mismatch: "取到的提交与固定提交对不上",

    /* ---------- worker lane 载体功能 ---------- */
    general_execution: "通用执行",
  };

  const warned = new Set();

  function t(value) {
    if (value === null || value === undefined || value === "") return "-";
    const key = String(value);
    if (Object.prototype.hasOwnProperty.call(dict, key)) return dict[key];
    if (!warned.has(key)) {
      warned.add(key);
      console.warn(`[i18n-zh] 未映射的枚举值：${key}`);
    }
    return key;
  }

  window.AIMAC_I18N = {dict, t};
})();
