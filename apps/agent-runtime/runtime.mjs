...
const JOIN_TOKEN_ORIGIN = "加入令牌由控制台签发：项目节点到「项目管理」→「注册运行节点」，组织共享节点到「组织管理」→「注册共享运行节点」；保存成文件后传 --join-token-file";
...
function explainAgentFailure(error) {
  ...
  const known = {
    join_token_invalid: "这张入网票不对（或已经不在服务端了）—— 找项目管理员进入目标项目「项目管理」→「注册运行节点」重新签发一张",
    join_token_expired: "这张入网票已经过期 —— 找项目管理员进入目标项目「项目管理」→「注册运行节点」重新签发",
...
async function status(config) {
  ...
  process.stdout.write(`节点 ${node.nodeName || config.nodeName}（${node.nodeId || config.nodeId}）：状态 ${node.status || "?"}，准入 ${node.admission || "?"}，最近心跳 ${heartbeat}，角色 ${(node.allowedRoles || config.allowedRoles || []).join("、") || "-"}，在跑派发 ${(node.activeDispatchIds || []).length} 个（累计完成 ${node.completedDispatchCount || 0}、失败 ${node.failedDispatchCount || 0}）${node.runtimeOutdated ? "；运行时版本已过旧，重新执行安装命令即可更新" : ""}\n`);
