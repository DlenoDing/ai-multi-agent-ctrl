...
const AGENT_RUNTIME_UNCOVERED_CEILING = 4;
...
// 两个调用点各有自己的实测值，数字与另两处（两套 e2e）一起放在 lib 里 ——
// 它们会同时变，散在四个文件里的后果是清一个集合要被顶红三次。
const UNCOVERED_COLLECTION_CEILINGS = UNCOVERED_CEILINGS;
...
function verifyGuidanceNamesRealPages(output) {
  ...
  // 【这道门够不着的一半，如实写在这里】：只认带位置词的三种形状（「X」页 / 「X」面板 /
  // 点「X」·「X」处·「X」按钮）。不带位置词的裸「X」认不出来 —— agentctl 里那句
  // "项目管理界面上「发加入令牌」"就是这么漏过去的（页名和控件名两个都不存在）。
  // 量过为什么不堵：报文里共 174 处「X」，其中 72 种不在页/面板/控件名表里，
  // 而绝大多数是概念引用（「不做过度兼容」「理论上更快/更稳」「一条都没有」）。
  // 按成员资格判会产出 70 条假警报，而假警报的下场是被人随手登记掉，登记就此失去意义。
  // 现在的办法是【写的时候就用带位置词的形状】，本仓的指路已全部改成这样。
...
async function verifyTestServersDieWithTheirParent(output) {
  ...
  try {
    ...
    const childPid = await new Promise((resolve) => {
      ...
      // 20 秒而不是 8 秒：正常情况下这一步是瞬时的（父进程 spawn 完立刻把 pid 打出来），
      // 但机器上同时跑着别的门时 node 起进程会被拖慢 —— 今天两次假红都是这么来的，
      // 报出来的还是"没能起来被测的服务端"，读的人会去查一个不存在的启动问题。
...
function verifyPerScopeRecordsSurviveTheirCap(output) {
  ...
  //
  // 【为什么不加一道「一轮编排必须随规模线性」的墙钟判据】：试过，撤了。
  // 用比值而不是绝对值（1000 单元 vs 4000 单元，线性约 4 倍、平方约 16 倍）本来是对的方向，
  // 但实测健康时 5.2~6.1 倍、把守卫改回去也只有 7.5 倍 —— 两者太近，定不出既不漏也不假红的阈值。
  // 要拉开到 10 倍得用 8000 单元（3.4s vs 0.36s），而这道判据会在变异门四路并行下被每条变异跑到，
  // 那既太贵又会重新引入负载敏感的假红（同一天已经为一条墙钟断言付过代价）。
  // 下面这几条「裁过就必须记下裁完剩多少」是确定性的，且正好卡住这一族的根因，够用。
