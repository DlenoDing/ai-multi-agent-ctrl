...
const child = spawn(process.execPath, ["apps/control-plane-ui/server.mjs"], {
  ...
  env: {
    ...
    AIMAC_LOCAL_SEED_ORG_ADMIN_TOKEN: "doctor-org-admin-token",
...
try {
  ...
  const defaultOrgView = await jsonFetch(port, "/api/state?view=orgs", {
    headers: {authorization: `Bearer ${defaultOrgAdminLogin.payload.sessionToken}`}
  });
  ...
  // 放在「合法取值要列出」那条之后：两者都读拒绝报文里的 supported，排在前面会让那条老登记失去判别力证明。
  // 【拼错的权限要被拒并点名】。原先只有安全筛：project:veiw 一路存进授权，回执成功，实际什么都打不开。
  {
    const typo = await jsonFetch(port, "/api/access-grants", {
      method: "POST", headers: {"Idempotency-Key": "doctor-typo-permission-grant", authorization: systemAuth},
    ...
    const typoInvite = await jsonFetch(port, "/api/accounts", {
      method: "POST", headers: {"Idempotency-Key": "doctor-typo-permission-invite", authorization: systemAuth},
    ...
    const vocab = await jsonFetch(port, "/api/state", {headers: {authorization: systemAuth}});
  ...
  // （概览按 active 列、编排跳过 archived）—— 新组从此没人看、没人推。
  {
    ...
    // 【拼错的角色范围要在签票时就拒】（放在「在用项目签得出票」之后：归档判据被改成全挡时，先红的得是那一句）。原先照收：票显示为已签发，节点拿它注册时才被拒 —— 失败推迟到另一台机器上。
    {
      const typoScope = await jsonFetch(port, "/api/agent-join-tokens", {method: "POST",
        ...
        body: JSON.stringify({projectId: liveProjectForCleanup, allowedRoles: ["agent-runtim"], ttlSeconds: 600})});
      if (typoScope.response.status !== 400 || typoScope.payload?.error !== "join_token_role_not_registered" || !(typoScope.payload?.unknownOwnerRoles || []).includes("agent-runtim")) {
  ...
  // 【终态之后那几个杠杆还按得动】—— 这条在【契约门】里验（taskGroupRuntimeControlRefusal
  // 与人工指令消费循环各一条）。这里不重复造：e2e 里造不出「已关闭的任务组」——
  // 关闭要求整条交付链走完（新建的空组被 all_required_work_closed 挡住，实测状态停在 intake），
  // 而为这一条断言把一整条交付跑通，代价与收益不成比例。如实登记，不留一条空转的断言。
  ...
  // 主体断言已经失败时【跳过】这两组扫描 —— 与限流那一组同一道保护，理由也一样：
  // 它们跑在 finally 里，主体抛错时照样会跑，而它们自己一旦也抛，新错误就把【真正的失败原因】
  // 替换掉了。完整变异门当场抓到过：会话令牌不逐个比对摘要那条变异，真因在登出那条断言上，
  // 被这里盖了个严实，报出来的是"某些写路由空 body 也成功了"。
  if (!mainBodyCompleted) {
