...
try {
  ...
  const stateStore = execFileSync("docker", ["compose", "exec", "-T", "postgres", "psql", "-U", "aimac", "-d", "aimac", "-t", "-A", "-c", "select concat(jsonb_typeof(state), '|', state->'runtime'->'storage'->>'stateStore') from aimac_control_plane_state where id='default';"], {cwd: root, encoding: "utf8"}).trim();
  if (stateStore !== "object|postgresql") throw new Error(`compose PostgreSQL state-store not active: ${stateStore}`);
  const doctor = spawnSync("npm", ["run", "agentctl", "--", "doctor", "--server=http://127.0.0.1:4317"], {cwd: root, encoding: "utf8"});
  ...
  spawnSync("docker", ["compose", "down", "-v"], {cwd: root, encoding: "utf8", stdio: "pipe"});
}

function run(command, args, options = {}) {
  execFileSync(command, args, {cwd: root, stdio: "pipe", timeout: options.timeout || 60000});
}
