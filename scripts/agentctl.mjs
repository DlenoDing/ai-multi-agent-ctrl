...
if (process.argv.slice(2).some((arg) => arg === "--help" || arg === "-h")) {
  ...
  console.log("  · 接一台新机器通常不用它：到目标项目的「项目管理」→「项目 Agent」→「注册项目节点」面板点「签发一次性加入令牌」");
  process.exit(0);
}
if (action !== "join-token create" && action !== "nodes list" && action !== "doctor") {
  fail(`认不出这个子命令${action ? `：${action}` : "（一个都没给）"}`,
    ["可用：agentctl join-token create | nodes list | doctor",
     ...
     // （真名：目标项目的「项目管理」→「项目 Agent」→「注册项目节点」面板 →「签发一次性加入令牌」）。
     // 写成【页 + 面板 + 点某按钮】这三种形状，指路核对那道门才看得见它。
     "接一台新机器通常不用它：到目标项目的「项目管理」→「项目 Agent」→「注册项目节点」面板点「签发一次性加入令牌」，"
       + "会直接给出可粘贴的安装命令"]);
}
