...
schemas_without_state_machine = Set.new(%w[
  ...
  AuthSession Agent McpCall RoomParticipant RuntimeIssueSample TestResult ExternalUpgradeImport McpProbeNode
])
...
# 状态分片与事件段说的是同一个项目的同一批数据，磁盘名字必须由【同一份】算法算出来。
# 这条判据原先在两个存储文件里各找一遍 "p_${createHash" 字面量 —— 那是在盯写法：
# 算法合成一份放进 project-paths.mjs 之后它当场变红，而行为一个字没变。
# 现在盯的是性质：算法在真相源里、两侧都 import 它、并且谁都不许再自带一份。
project_paths_source = read_source("apps/control-plane-ui/lib/project-paths.mjs")
errors << "project shard filenames must use bounded hash ids and preserve legacy reads" unless project_paths_source.include?("p_${createHash") && project_paths_source.include?("function legacySafeProjectId")
[["state-store", state_store_source], ["project-event-store", project_event_store_source]].each do |name, source|
  errors << "#{name} must take project directory names from lib/project-paths.mjs" unless source.include?("from \"./project-paths.mjs\"") && source.include?("safeProjectId(") && source.include?("legacySafeProjectId(")
  errors << "#{name} must not keep its own copy of the project directory naming" if source.include?("function safeProjectId") || source.include?("function legacySafeProjectId")
end
...
# H3: the derived-task-request producer must emit the schema-required fields and traverse the modeled lifecycle.
errors << "derived-task-request producer must conform to its schema" unless core_source.include?("schemaVersion: \"derived-task-request/v1\"") && core_source.include?("proposedInsertionMode: \"current_absorb\"") && core_source.include?("status: \"candidate\"") && core_source.include?("\"classified\", \"absorbed\", \"orchestrator\"") && core_source.include?("derivedRequest.status = \"absorbed\"")
...
errors << %(待办统计必须按处置权限过滤（否则出现永远清不掉的红点）) unless app_js_source.match?(/const add = \(id, label, page, (?:workspace, )?items, allowed[,)]/)
...
if ops_section.nil?
  ...
  ops_sources = ["apps/control-plane-ui/server.mjs", "apps/control-plane-ui/lib/state-store.mjs",
    "apps/control-plane-ui/lib/control-plane-core.mjs", "apps/control-plane-ui/public/app.js",
