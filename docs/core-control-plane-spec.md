...
下表只把已实现、可调用的 HTTP 路由作为主入口。标为“非入口设计项”的对象仍然存在于 schema、MCP 或内部编排链中，但不得由 Agent 按该 HTTP 路径直接调用；机器执行器必须使用同一行说明里的真实入口。

| 方法 | 路径 | 作用 | 允许 actor |
| --- | --- | --- | --- |
| POST | `/api/projects` | 创建项目 | orchestrator |
| GET | `/api/projects/:projectId` | 读取项目详情、任务组窗口、任务组总数和仓库产出摘要；普通账号对不可见/不存在项目返回同一拒绝口径 | orchestrator、scheduler、agent-runtime、monitor、admin read-only |
...
| GET | `/api/task-groups/:taskGroupId` | 读取任务组详情、工作项窗口、session/dispatch 摘要、仓库产出、最近 readiness 和 close barrier | orchestrator、scheduler、agent-runtime、monitor、admin read-only |
...
| POST | `/api/effective-instruction-packets` | **非入口设计项**：有效指令包由派发链内部生成；如需外部创建压缩信封，使用 `/api/instruction-envelopes` | orchestrator、policy-engine |
| POST | `/api/instruction-envelopes` | 创建压缩后的角色指令信封 | orchestrator、instruction-optimizer |
| POST | `/api/role-drift-guards` | **非入口设计项**：角色漂移防护对象随派发链落账；治理入口使用 MCP `governance-mcp.role_drift_guard_bind` 或任务组控制 `rebound_drift` | orchestrator、monitor |
| POST | `/api/role-drift-guards/:guardId/rebound` | **非入口设计项**：纠偏经 `/api/task-groups/:taskGroupId/control` 的 `rebound_drift` 或 MCP `governance-mcp.role_drift_rebound` 承载 | orchestrator |
...
| POST | `/api/model-selection-decisions` | **非入口设计项**：选择记录由 `/api/model-selection/decide` 一并落账；不得单独伪造已选择记录 | model-registry、scheduler |
...
| POST | `/api/completion-readiness/compute` | **非入口设计项**：完成就绪经 `GET /api/task-groups/:taskGroupId/readiness` 或 MCP `review-mcp.completion_readiness_compute` 计算 | orchestrator、monitor |
...
| POST | `/api/commands` | **非入口设计项**：Agent 控制命令经节点控制 API/MCP 创建，Agent 只通过 `/api/agent/v1/control` 长轮询接收 | orchestrator、command-bus、agent-runtime |
...
| `governance-mcp` | `approval_request_create`、`policy_decision_eval`、`finding_submit`、`contract_publish`、`effective_instruction_create`、`role_drift_guard_bind`、`role_drift_rebound`、`rule_source_resolve`、`runtime_issue_pattern_submit`、`system_upgrade_candidate_export`、`system_upgrade_external_import`、`close_barrier_compute`、`finding_resolve`、`approval_resolve` |
