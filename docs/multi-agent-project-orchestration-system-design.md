...
本文是系统终态蓝图。该系统面向 AI 模型、AI Agent 和程序执行器，不面向非系统执行路径。入口总控会话接收目标、边界和不可编程的外部能力信号；后续拆解、调度、执行、复验、提交、推送、发布准备、规则沉淀和关闭判断都必须由 Orchestrator、Decision Center、Scheduler、Agent Runtime、MCP Proxy、Policy Engine 和角色化 AI Agent 自动完成。
...
后续编码时以 `spec/` 下机器可解析文件为执行优先级最高的项目内契约；自然语言文档只解释设计意图。
...
| `needs_attention` | 需要总控调度、AI 决策或外部能力边界信号 |
...
3. OS 安全弹窗、UAC、macOS TCC、浏览器账号登录、第三方 OAuth consent 等不能假设可自动批准；系统只能检测、截图/摘要、回传为 `external_capability_required`，然后使用预授权能力、改派、降级或中止。
...
Agent 加入系统必须做到“总控生成、Agent 执行一条命令、系统自动初始化”。不要要求 Agent Runtime 之外的配置动作填写仓库、MCP、数据库、规则路径和资源参数。
...
| P0 | 轻量 Policy Engine | 需要 action/resource/context 级策略，判断哪个 session 能对哪个 project、repo、MCP tool、DB、环境执行什么动作；可由系统内置表和代码执行，也可按指标替换为专用 policy engine |
| P0 | Secret 生命周期 | Secret ref 必须支持发放、租约、轮换、撤销、泄漏扫描、最小 scope 和审计；可由加密 secret 表或专用 secret manager 实现 |
...
| P1 | 主动告警 | 用 `alert_rules/alerts`、webhook/email 或专用观测系统实现 heartbeat、DLQ、磁盘、DB、Artifact、备份和错误率告警 |
...
1. 所有 required WorkItem 已 closed，或有入口总控会话记录的目标边界调整。
