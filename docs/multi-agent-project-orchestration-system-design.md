...
本文是系统终态蓝图。为了避免设计过大导致第一版无法落地，当前仓库已把实施入口拆为以下配套文档：

| 文档 | 定位 |
| --- | --- |
| [MVP v0.1 范围定义](mvp-v0.1-scope.md) | 收敛第一版必须实现的闭环、验收场景和明确不做事项 |
| [核心控制平面规格](core-control-plane-spec.md) | 固化 v0.1 数据库核心表、HTTP API、MCP tools、事件模型和事务边界 |
| [Agent Runtime 协议](agent-runtime-protocol.md) | 定义 Agent 一键加入、初始化、心跳、session、artifact、权限阻断和断线恢复 |
| [实施待办拆分](implementation-backlog.md) | 把设计拆成 P0/P1/P2 编码任务、依赖和验收标准 |

后续编码时以这些实施文档为近期边界，本文保持为架构基线和终态约束。

## 1. 设计目标

本系统目标是把一个项目从最初需求、方案、产品设计、UI 设计、前后端开发、测试、发布、线上质量验收到后期维护中的新功能、功能修改和 bug 修复，统一纳入一个由总控启动和调度的多 Agent、多会话、多子 Agent 协作系统。

系统必须满足：

1. 人工只从总控入口下达目标、补充约束和审批关键动作。
...
7. 系统内置初始 skill/rules，并能在实际运行中把稳定问题、协作流程、角色要求、业务规则沉淀为版本化规则，约束后续会话。
...
这些经验不是 MGP 特例，应作为系统默认规则。

## 3. 总体架构

```mermaid
flowchart LR
    Human["人工输入"] --> OC["总控 Orchestrator"]
    OC --> DC["决策中心"]
    OC --> RM["角色/会话调度器"]
    OC --> MON["监测与度量"]
    RM --> AG["Agent Gateway"]
...
| 模块 | 职责 |
| --- | --- |
| Orchestrator 总控 | 唯一人工入口、目标拆解、任务组创建、角色分配、阶段推进、冲突裁决、关闭屏障 |
...
| TaskGroup | 围绕一个目标建立的协作单元，例如新项目建设、MGP core-init、某次版本发布 |
...
| AgentTrustEvent | Agent 运行期可信度变化、违规、降级、隔离和人工解封记录 |

## 5. 总控职责

总控是唯一人工指令入口，但不应亲自做所有实现。它的主要职责是：
...
3. 一个 session 可以启动内部子 Agent，但子 Agent 只为该 session 服务，不直接拥有全局任务。
4. 独立 reviewer 必须是未参与对应实现批次的 session。
5. 同职责连续工作优先复用同一个 session，避免上下文和状态割裂。
6. 新 session 默认使用最小上下文：规则 locator、任务 locator、输入数据、写入面、输出格式和停止条件。
...
```mermaid
flowchart TD
    A["人工目标输入"] --> B["总控建立 Project/TaskGroup"]
    B --> C["需求与现状分析"]
    C --> D["产品方案和验收口径"]
    D --> E["总体架构和技术方案"]
    E --> F["UI/UX 设计"]
...
| 阶段 | 关键输出 | 关闭条件 |
| --- | --- | --- |
| Intake | 项目目标、范围、约束、成功标准 | 人工确认输入足以启动分析 |
...
| 字段 | 值 |
| --- | --- |
| `deliveryStage` | `intake`、`discovery`、`product_design`、`solution_design`、`ui_design`、`development`、`global_development_review`、`verification`、`repair`、`reverification`、`release`、`online_quality`、`closed` |
...
2. 可由系统授权的权限，例如 MCP grant、Git credential、secret lease、网络 allowlist，可以由总控和审批策略决定后下发。
3. OS 安全弹窗、UAC、macOS TCC、浏览器账号登录、第三方 OAuth consent 等不能假设可自动批准；系统只能检测、截图/摘要、回传和等待人工或改派。
4. PermissionRequest 必须包含 `promptType`、进程/窗口、请求资源、风险级别、截图或日志 artifact、当前阻断 work、可选处理方案和安全重试点。
5. 超时未处理时，总控必须选择继续等待、改派到已授权 Agent、降低任务范围或中止 work，不能让 session 长期空等。
...
```json
{
  ...
  "type": "checkpoint|delta|finding|decision|question|answer|blocker|handoff|review_request|review_result|progress|command",
  "priority": "normal|important|p0",
  "correlationId": "corr_...",
  "idempotencyKey": "task-checkpoint-...",
  "stateVersion": "CORE-INIT-DEV-20260723.5",
  "body": {
    "summary": "...",
    "locators": [],
    "expectedAction": "read_and_continue"
  },
...
初始化结果写入系统库并生成 `agent_profile_digest`。Scheduler 后续只基于最新画像分配任务；画像过期、心跳中断或资源低于阈值时，节点只能接收只读轻任务或进入隔离。
...
3. 当额度明显不足以完成任务时，Scheduler 不派发大任务，改派短 reviewer、只读扫描、轻量文档或等待补充额度。
...
| `manual_required` | 关键阶段必须由人工或总控显式指定模型 |
...
### 10.6 Agent 一键加入脚本和最简指令

Agent 加入系统必须做到“总控生成、Agent 执行一条命令、系统自动初始化”。不要要求 Agent 人工填写仓库、MCP、数据库、规则路径和资源参数。
...
给 Agent 主机执行的最简指令。受信开发环境可以使用管道方式，生产环境必须使用后面的校验版：
...
```text
在目标主机执行下面的一条加入命令。不要手动 clone 仓库，不要手动配置 MCP、数据库或密钥。
...
| `manual_unblock` | 人工复核 runtime、日志、配置和证据后解封 |
...
| OAuth/账号登录 | 通常需要人工 | 使用 device code、OAuth URL 或浏览器登录状态；Agent 只回传登录链接/截图/影响面，不保管长期账号密码 |
| OS 安全弹窗 | 通常不能自动批准 | macOS TCC、Screen Recording、Accessibility、Windows UAC、Keychain、Linux sudo/polkit 必须预检或人工处理 |
| 浏览器权限弹窗 | 低风险可策略化 | download、clipboard、camera、mic、location 等按项目策略；高风险默认回传总控 |
| 付费/额度确认 | 需要策略或人工 | 外部付费 API、模型额度升级、云资源创建必须走 ApprovalRequest |
...
5. 总控收到后可选择：发放系统权限、创建 ApprovalRequest、要求人工在该主机操作、改派到已授权 Agent、缩小 scope、跳过非必要步骤或中止 work。
...
| `permission_requests` | `id,project_id,task_group_id,work_item_id,session_id,agent_node_id,prompt_type,requested_capability,requested_resource,risk_level,status,safe_retry_point,artifact_ref,created_at` |
...
任务组必须允许配置模型使用策略。策略可以全局生效，也可以按角色、阶段或风险级别覆盖。
...
以下对象是终态系统必须具备的治理层，不要求 MVP 一次性做成复杂平台，但数据库模型要提前留好边界。
...
| `approval_requests/approval_decisions` | 把人工审批变成可等待、可超时、可审计、可回放的状态机 |
| `audit_logs/audit_digest_batches` | 用 append-only 和 hash chain 支撑控制面、读写操作和管理员行为审计 |
| `alert_rules/alerts` | 在不引入复杂监控系统的前提下形成主动告警闭环 |
| `agent_trust_events/agent_reliability_scores` | 支撑 Agent 运行期降级、隔离、额外复验和人工解封 |
...
| --- | --- |
| `git_repo` | `MGP/stock-data-service` |
...
| Skill | 作用 |
| --- | --- |
| `project-intake` | 把人工目标转为项目/任务组/成功标准 |
...
18. 待审批请求、审批 quorum、超时策略和人工范围调整。
...
-> 选择 MODEL + REASONING_LEVEL
-> 记录 model_selection_event
-> 创建或复用 WorkSession
```
...
| `fast` | 搜索、扫描、简单 bug、小范围文档、重复性 guard、格式/配置小改 | 高并行，但必须有明确 scope 和自动/人工复验 |
...
while taskGroup.goalExecutionStatus != closed:
  ...
  if hasReviewRequest(events):
    createIndependentReviewer(events)
    continue
  ...
  for work in readyWork:
    ...
    if candidate and resourceManager.canClaimProjectLease(work.writeScope, projectLocks):
      assignWork(work, candidate.agentNode, candidate.model, candidate.reasoningLevel)

  checkpoints = collectNewCheckpoints(events)
  updateProgressAndQualityGates(checkpoints)

  if closeBarrierSatisfied(state):
...
| Close Gate | 所有 required 项完成或有明确外部阻断和人工范围调整 |
...
8. MVP 使用本地文件系统也必须做 `artifact_verify` 和 `artifact_gc`：校验 DB metadata 与文件 digest 一致，按项目 quota 和 retention 清理。

## 18. 成本和上下文控制

系统内置以下成本控制：

1. 稳定任务契约首次下发，后续只发短 delta。
...
12. MVP 必须内置主动告警，不只提供看板。至少覆盖 Agent heartbeat 中断、DLQ 增长、outbox backlog、lease 长期未释放、磁盘水位、DB 连接耗尽、Artifact 写入失败、备份失败、错误率突增和模型/工具连续限流。
...
20. Agent 触发信任降级后，写入型权限、secret、Git push、部署和高风险 MCP grant 默认撤销，人工解封前只能执行恢复和证据上传动作。
...
3. 不可逆迁移必须标记 `reversible=false`，走高风险审批，并提供备份、恢复点和人工确认。
...
5. CI 必跑项、失败重跑权限、跳过条件和人工 override 必须写入质量门。
...
3. 人工审批入口。
4. 规则变更和决策记录视图。
5. MVP 单机部署 runbook、升级/回滚命令和健康检查。
6. 备份、恢复、RPO/RTO、HA 和灾难恢复演练入口。
7. 告警规则、告警处理、静默和升级入口。

目标：让人可以实时掌控，而不是翻多个会话。

## 24. 实施级缺口和必须补齐项

当前设计已经覆盖协作模型，但要变成可长期运行的平台，还必须补齐以下实施层能力。这里列的是必须具备的功能，不代表前期必须引入复杂外部系统；MVP 先用 TypeScript 控制服务和 PostgreSQL 实现，等真实规模、并发和运维压力证明需要后再替换为专用组件。
...
| P0 | MCP Proxy | MCP tool 不直接暴露给会话，必须经过 proxy 做授权、审计、限流、脱敏、schema 校验、结果截断和 idempotency |
| P0 | 轻量 Policy Engine | 需要 action/resource/context 级策略，判断哪个 session 能对哪个 project、repo、MCP tool、DB、环境执行什么动作；MVP 用系统内置表和代码执行 |
| P0 | Secret 生命周期 | Secret ref 必须支持发放、租约、轮换、撤销、泄漏扫描、最小 scope 和审计；MVP 用加密 secret 表实现 |
| P0 | Outbox/Inbox/DLQ | Room 消息、command、MCP 调用、Git 操作都要可重放、可去重、可进入死信队列，不能依赖 WS 在线状态 |
| P0 | Agent Admission | 资源、额度、工具、MCP、Git、项目访问画像必须有过期、可信度、降级和隔离策略 |
| P0 | 跨任务组资源仲裁 | 同一项目下多个任务组共享 repo、DB、环境、release 分支、MCP 写工具和部署目标时，必须由项目级 lease 和 freeze window 仲裁 |
| P0 | Contract Registry | API、DB schema、消息、配置和设计 token 等契约必须有版本、digest、消费者绑定和兼容性检查 |
| P0 | 级联失效和复验 | 上游契约、规则、环境、migration 或输入 digest 变化时，必须精确标记 affected work 为 invalidated/reverify_required |
| P0 | Integration Batch | 多会话并行产物进入主线前必须经过 ChangeSet、merge queue、batch CI、冲突 owner 和 release manifest |
| P0 | Command Effects | 所有副作用命令必须记录 before/after、外部操作 ID、可逆性、rollback 和 verify 结果 |
| P0 | 审批工作流 | 高风险动作、范围变更、生产操作和批量授权必须有 ApprovalRequest 状态机、quorum、超时和审计 |
| P0 | Permission Gateway | Agent 遇到弹窗、OAuth、OS 权限、MCP/Git/DB/网络授权缺失时，必须结构化回传并由总控/决策中心裁决 |
| P0 | 安装供应链 | Agent 安装脚本、runtime、MCP server 包和镜像必须固定版本并校验签名/hash，不使用 `latest` 作为生产入口 |
| P1 | Multi-repo Checkpoint | 一个 run 可绑定多个仓库 commit、文档、配置和证据；不追求跨仓原子提交，但必须可恢复 |
| P1 | Knowledge Index | rules、项目文档、checkpoint、evidence 和 decision 需要全文索引和语义索引，减少重复读长文档 |
| P1 | 平台自测 | 需要 fake agent、fake MCP、消息乱序、断线恢复、lease 过期、重复 command、权限拒绝等测试矩阵 |
| P1 | HA/备份 | PostgreSQL、Artifact、Room Broker、Agent Gateway、MCP registry 都要备份、恢复和演练 |
| P1 | 主动告警 | MVP 用 `alert_rules/alerts` 和 webhook/email 实现 heartbeat、DLQ、磁盘、DB、Artifact、备份和错误率告警 |
| P1 | 版本兼容 | Control Plane、Agent Runtime、MCP tool 和 command schema 必须有 protocolVersion、minRuntimeVersion、capability flags 和废弃窗口 |
| P1 | Execution Environment | 测试、E2E、性能和发布验证必须绑定 environment snapshot，记录镜像、seed、migration、feature flag 和配置 |
| P1 | Artifact 生命周期 | 本地 Artifact 也必须有项目级 quota、retention、redaction、digest verify、GC、备份和访问审计 |
| P1 | 日常研发 SOP | 分支、PR、commit、CI、schema/codegen、环境变量、seed data 和依赖变更必须项目规则化 |
| P2 | UI 操作台细化 | 任务 DAG、Agent 资源、额度、MCP grant、Git checkout、quality gate、evidence 需要一屏可见 |
...
| 层 | 推荐 |
| --- | --- |
| 人员/服务身份 | MVP 用系统内置账号、角色和 service token；后期再接 OIDC/SSO |
| 细粒度资源授权 | MVP 用 PostgreSQL policy 表和代码内 enforcement；后期再评估 OPA/OpenFGA |
| 密钥管理 | MVP 用 PostgreSQL encrypted secret 表；后期再接 Vault 或云 Secret Manager |
...
如果 command 超时、Agent 断线或重试，系统必须先查 `command_effects` 和外部 operation id，再决定重试、补偿或交人工裁决。
...
| OAuth 或第三方账号授权 | 回传 device code、授权 URL、窗口截图和影响面，由人工或预配置 service account 处理 |
| 浏览器普通权限 | 对 download、clipboard 等低风险项可按项目白名单自动处理，高风险项回传 |
| OS 安全弹窗 | 只能预检、检测、截图、回传和等待人工；不能假设可自动点击同意 |
| sudo/UAC/Keychain | 应在 Agent 安装和 admission 阶段解决；运行期出现则暂停副作用并请求人工或改派 |
...
-> grant|approval|manual_action|reassign|reject
...
4. OS 或第三方平台明确要求人工同意时，系统不得伪装成人工或绕过安全边界。
5. 频繁出现的权限阻断要回流到 Agent admission 和项目运行配置，后续在任务派发前预检。
...
7. 解封必须有人工复核或可信自动证据，包括 runtime digest、日志、outbox、checkout 和 artifact manifest。
...
| RPO/RTO | MVP 明确目标，例如 RPO 24h、RTO 4h；关键项目可提高频率 |
...
前期原则：少组件、低成本、能完整闭环。所有核心能力先由一个控制服务、一个 PostgreSQL 和一个 Agent Runtime 完成；不要一开始引入 Temporal、NATS、Kafka、Vault、OPA、OpenFGA、MinIO、Prometheus/Grafana/Loki 这类专用基础设施。

| 层 | 推荐 |
| --- | --- |
| 语言和仓库 | TypeScript monorepo |
| Control Plane/API | Node.js + Fastify，复杂团队偏好装饰器时才选 NestJS |
| Agent Runtime | TypeScript/Node.js，先保持同语言；后期需要单二进制和更强进程控制时再改 Go |
...
| Cache/Presence | PostgreSQL 表起步；只有在线状态压力明显后再加 Redis |
...
| Auth | 系统内置用户/session token/service token；公网 Agent 用短期 join token + 后续 mTLS/JWT |
| Policy | PostgreSQL policy 表 + 代码内 enforcement |
| Secret | PostgreSQL encrypted secret 表，master key 来自环境变量或宿主 keychain |
| Observability | JSON structured logs + PostgreSQL metrics/audit/alert 表 + webhook/email |
| E2E | Playwright，作为测试依赖，不作为平台基础设施 |
| UI | React + TanStack Query，直接由控制服务托管静态资源 |
| 部署 | 单机 Docker Compose：control-plane、PostgreSQL、可选 agent-runtime |

### 25.1 MVP 单机部署运行手册

MVP 必须交付可执行 runbook，而不是只给架构图。
...
| 初始化 | `db migrate`、创建 admin、生成 master key、创建第一个 project、生成 join token |
| 升级 | 先备份，再迁移 DB，再灰度 Agent runtime，失败按 runbook 回滚 |
| 验收 | 重启后验证登录、room send/wait、agent heartbeat、checkpoint、artifact upload、MCP proxy、backup smoke |
...
1. 前期只有 TypeScript、Node.js、PostgreSQL、SQLite、WebSocket、本地文件系统和 Git，部署成本最低。
...
4. PostgreSQL outbox + `FOR UPDATE SKIP LOCKED` 足以支持早期 command、room message、MCP call 和 Git operation 的可靠执行。
5. Agent Runtime 前期用 TypeScript 可以复用协议、schema、MCP adapter 和安装包逻辑；后期节点数量多或跨平台分发困难时，再把 runtime 重写为 Go。
6. Secret 和 Policy 前期放在系统内部实现，降低部署复杂度；但接口要抽象好，后期可替换为 Vault、OPA 或 OpenFGA。
7. 监控前期用结构化日志和数据库指标足够定位问题；后期并发、SLO 和运维团队成熟后，再接 OpenTelemetry 和 Prometheus/Grafana。
...
11. 前期技术栈必须低成本、少组件、可完整闭环；复杂专用基础设施只能作为后期按指标升级项。
12. 规则版本化，历史不覆盖当前规则。
13. 修复和复验分离。
14. 等待依赖事件驱动，不空轮询。
...
1. 所有 required WorkItem 已 closed，或有人工确认的范围调整。
2. 所有 P0/P1 已根因修复或明确外部阻断。
...
| 运维/落地复审 | MVP 运行手册、备份恢复、告警、版本兼容、迁移、Artifact 容量和故障演练 |
...
互审输出必须进入 `review_report` artifact，并把被采纳的发现转为 WorkItem、Rule Candidate 或 Decision Record。未采纳发现也要记录原因，避免下一轮重复争论。
...
+ 事件驱动等待和成本控制
+ 前期低成本单体栈，后期按指标拆分专用基础设施
```

用这个结构，完整新项目、后期功能修改、新功能开发、bug 修复和线上质量闭环都可以走同一个机制，只是按风险和影响面缩放角色数量、验证深度和关闭条件。
