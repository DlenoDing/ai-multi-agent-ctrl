...
任务组可以设置动态模型偏好，但不得设置固定 profile 表：

| 策略 | 含义 |
| --- | --- |
| `dynamic_context` | 每次派发前由 integration owner 基于当前任务内容、风险、阶段门、依赖、writeSet 和是否需要裁决短分析后指定实际模型 |
...
Agent 加入系统必须做到“登录管理界面、进入目标项目、生成一次性 join token、受信执行环境运行 bootstrap、系统自动初始化”。不要要求 Agent Runtime 之外的配置动作填写仓库、MCP、数据库、规则路径和资源参数。join token 由有项目 `agent:activate` 权限的账号在项目管理页生成，必须绑定 project、expected node、allowed roles、MCP tool allowlist、ttl、maxUses 和创建者审计记录；常规入网流程不得依赖服务器命令行生成 token。
...
```text
由受信执行环境执行下面的一条加入命令。目标主机只完成 Agent Runtime 入网和 Runtime 托管的远程 MCP 配置刷新，不在 Agent Runtime 之外 clone 仓库、启动 MCP server、连接数据库或注入密钥。
...
| `task_group_model_policies` | `id,task_group_id,mode,dynamic_factors,allowed_models,role_constraints,reasoning_policy,parallelism_policy,status` |
...
### 11.4 动态模型指定和 DISPATCH v1

系统必须支持市面上常用模型和本地模型，不把任一供应商或单一模型写死为唯一执行路径。Model Registry 只维护可用模型与 Agent 能力库存，用于判断候选是否可运行；它不是派发依据的固定 profile 表，也不能把 `profile` 字段写入 dispatch envelope。

每次派发前，integration owner 必须基于当前任务内容和整体状态做短分析，并显式指定实际 `model` 与 `reasoning`。分析因素包括任务内容、风险等级、阶段门、依赖状态、writeSet、是否需要裁决、是否只是机械实现、定向测试或状态同步。分析必须短，通常一行，写入 `modelDecision`：

```text
modelDecision: fixed writeSet implementation; no architecture裁决; G05 source correction only -> gpt-5.5 / medium
```

每个 dispatch 必须使用固定 `DISPATCH v1` envelope，且 envelope 中只保留 `model`、`reasoning`、`modelDecision` 三个模型相关字段，不使用 `profile` 字段，不继承上一轮会话模型默认值。
...
base: data@3d6cb04 quote@abce5f4 trade@e2b7753c
writeSet:
- <exact paths>

read:
- AGENTS.md
- <server-issued dispatch package locator>
- <git-tracked project/task locator>
...
doNot:
- do not expand graph
- return to owner if writeSet/dependency changes
...
长背景、规则、契约、验收和共享定义不得重复粘贴到 prompt。跨主机/跨网络情况下的最优输入方式是：系统服务器通过 Agent Gateway 下发签名 dispatch package；项目代码、任务文档和产出 manifest 通过可访问 Git 仓库定位；大型稳定规则只放 `inputLocators`、digest 和 package 文件引用。Agent 无权访问的外部材料必须由总控先转成仓库内或服务器 package 内的稳定 locator 后再派发。

混合任务必须先拆分再分派。若 work item 同时包含深度分析/架构判断和代码开发/改造，总控先自动派生 `deep_analysis` 与 `implementation` 两个机器任务，分别写出各自的 `modelDecision`、`model` 和 `reasoning`。不得把详细深度分析和代码实现塞进同一 task contract，避免低模型分析不充分或高模型执行普通代码开发造成浪费。
...
MODEL_POLICY=<dynamic_context|auto_best|auto_fast|cost_aware>
...
MODEL_SELECTION_MODE=<dynamic_context|auto_best|auto_fast|cost_aware>
