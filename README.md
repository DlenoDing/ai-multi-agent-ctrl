...
入口总控会话只接收目标、边界和不可绕过的外部能力信号。目标进入系统后，项目拆解、任务组创建、角色实例化、模型选择、会话派发、代码修改、测试、复验、提交、推送、发布准备、权限阻断处理、证据校验、规则候选收集和关闭判断都必须由 AI Agent 和程序自动执行；运行期不得自动改写系统规则、角色、策略或控制面代码，系统升级结果只通过系统外维护后导入。
...
```text
系统管理员登录账号：system.admin@local  (在登录页「登录账号」处填它)
本地初始化令牌：...                    (与上面的登录账号配合使用)
```
